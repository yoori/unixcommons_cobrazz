...
{
public:
  int run() /*throw (eh::Exception)*/;

private:
  static const char* data_[][2];
};

const char* IDNANorm::data_[][2] = {{"http://a.com", "a.com"}, {"a.com", "a.com"},
  {"a.com/path", "a.com/path"}, {"a.com?query", "a.com?query"},
  {"a.com/path?query", "a.com/path?query"},
  {"http://a.com/path?query#fragment", "a.com/path?query"}, {"A.COM", "a.com"},
  {"http://пример.испытание/пример.испытание?пример.испытание",
    "пример.испытание/пример.испытание?пример.испытание"},
  {"http://ПрИмЕр.ИсПыТаНиЕ/ПрИмЕр.иСпЫтАнИе?пРиМеР.иСпЫтАнИе",
    "пример.испытание/ПрИмЕр.иСпЫтАнИе?пРиМеР.иСпЫтАнИе"},
  {"xn--e1afmkfd.xn--80akhbyknj4f/%33%5%20?%41%", "пример.испытание/3%5 ?A%"},
  {"Xn--E1AfMkFd.xN--80aKhByKnJ4f/%%33%5%20?%%41%", "пример.испытание/%3%5 ?%A%"},
...
int IDNANorm::run() /*throw (eh::Exception)*/
{
  for (size_t i = 0; data_[i][0]; i++)
  {
    try
    {
      std::string res(HTTP::keywords_from_http_address( String::SubString(data_[i][0])));
      if (data_[i][1])
      {
        if (data_[i][1] != res)
        {
    ...
    catch (const eh::Exception& ex)
    {
      if (data_[i][1])
      {
        std::cerr << "Failed to convert '" << data_[i][0] <<
          "' got exception " << ex.what() << " instead of '" << data_[i][1] << "'" << std::endl;
      }
    }
  }
  return 0;
...
int main()
{
  ...
  {
    std::cerr << " eh::Exception caught. Description:" << ex.what() << std::endl;
  }
