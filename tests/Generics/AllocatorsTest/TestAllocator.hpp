...
namespace Generics::Allocator
{
    ...
    public Base
  {
     ...
     */
    virtual Pointer allocate(std::size_t& n) /*throw (MemoryOut)*/ = 0;
     ...
     */
    virtual void deallocate(Pointer ptr, std::size_t) noexcept = 0;
     ...
     * value and return it.
     */
    std::size_t stat_allocate(bool get = false) noexcept;
     ...
     * value and return it.
     */
    std::size_t stat_deallocate(bool get = false) noexcept;
     ...
     * value and return it.
     */
    std::size_t stat_sys_allocate(bool get = false) noexcept;
     ...
     * value and return it.
     */
    std::size_t stat_sys_deallocate(bool get = false) noexcept;

    /**
     * Need this method, because system resources are releasing
     * by pools while destroying pools.
     */
    virtual void calc_sys_deallocate() noexcept = 0;
  ...
  protected:
    using MemoryBlock = std::pair<std::size_t, Pointer>;

    virtual ~BaseAllocator() noexcept = 0;

    Pointer alloc_size_block_(std::size_t n) /*throw (MemoryOut)*/;

    struct SizeBlockEraser
    {
      void operator() (const MemoryBlock& ptr) const noexcept;
    };

    const SizeBlockEraser& get_eraser_() noexcept;
  ...
  using BaseAllocator_var = ReferenceCounting::QualPtr<BaseAllocator>;

  struct DefaultModifier
  {
    static void modify(std::size_t& n, std::size_t align, bool pool = false) noexcept;
  };
  ...
  public:
    ...
    virtual Pointer allocate(std::size_t& n) /*throw (MemoryOut)*/
    {
      return new unsigned char[n];
    }

    virtual void deallocate(Pointer ptr, std::size_t) noexcept
    {
      delete[] static_cast<unsigned char*>(ptr);
    }

    virtual void calc_sys_deallocate() noexcept;
  };
  ...
  public:
    ...
    Pointer allocate(std::size_t& n) /*throw (MemoryOut)*/;

    void deallocate(Pointer ptr, std::size_t) noexcept;

    virtual void calc_sys_deallocate() noexcept;

    virtual ~PoolListSimple() noexcept;

  private:
    using MemBlocks = std::list<MemoryBlock>;
  ...
  public:
    ...
    virtual ~PoolMultiMapSimple() noexcept;

  private:
    using MemBlocks = std::multimap<uint32_t, void*>;
    MemBlocks pool_memory_blocks_;
    Sync::PosixMutex mutex_;
  };
  ...
  private:
    ...
    using Allocator = PoolListSimple<DefaultModifier, 0>;
...
namespace Generics::Allocator
{
  void DefaultModifier::modify(std::size_t& n, std::size_t /*align*/, bool pool) noexcept
  {
    if (pool)
    {
      n += sizeof(std::size_t);
  ...
  void Reserve::modify(std::size_t& n, std::size_t /*align*/, bool pool) noexcept
  {
    n <<= 1;
    if (pool)
    {
  ...
  void Aligned::modify(std::size_t& n, std::size_t align, bool pool) noexcept
  {
    if (pool)
    {
      n += sizeof(std::size_t);
  ...
  void AlignedReserve::modify(std::size_t& n, std::size_t align, bool pool) noexcept
  {
  ...
  inline BaseAllocator::BaseAllocator() noexcept
    : stat_allocate_(0),
      stat_deallocate_(0),
  ...
  inline const BaseAllocator::SizeBlockEraser& BaseAllocator::get_eraser_() noexcept
  {
    return eraser_;
  }

  inline std::size_t BaseAllocator::stat_allocate(bool get) noexcept
  {
    return get ? stat_allocate_ :
      __gnu_cxx::__exchange_and_add(&stat_allocate_, 1);
  }

  inline std::size_t BaseAllocator::stat_deallocate(bool get) noexcept
  {
    return get ? stat_deallocate_ :
      __gnu_cxx::__exchange_and_add(&stat_deallocate_, 1);
  }

  inline std::size_t BaseAllocator::stat_sys_allocate(bool get) noexcept
  {
    return get ? stat_sys_allocate_ :
      __gnu_cxx::__exchange_and_add(&stat_sys_allocate_, 1);
  }

  inline std::size_t BaseAllocator::stat_sys_deallocate(bool get) noexcept
  {
  ...
  inline
  BaseAllocator::Pointer BaseAllocator::alloc_size_block_(std::size_t n) /*throw (MemoryOut)*/
  {
    stat_sys_allocate();
  ...
  inline void BaseAllocator::SizeBlockEraser::operator() (const MemoryBlock& mb) const noexcept
  {
    ...
    delete[] (static_cast<unsigned char*>(mb.second) - sizeof(std::size_t));
  }
  ...
  FakeAllocator<SizeModificator, align>::allocate( std::size_t& n) /*throw (MemoryOut)*/
  {
    stat_allocate();
  ...
  template<typename SizeModificator, size_t align>
  void FakeAllocator<SizeModificator, align>::calc_sys_deallocate() noexcept
  {
  }
  ...
  inline
  BaseAllocator::Pointer
  Simple<SizeModificator, align>::allocate( std::size_t& n) /*throw (MemoryOut)*/
  {
    stat_allocate();
    stat_sys_allocate();
    SizeModificator::modify(n, align);
    ...
    Pointer ptr = new unsigned char[n];
    trace_message("Generics::Simple::allocated(): ", reinterpret_cast<size_t>(ptr));
    return ptr;
  };

  template<typename SizeModificator, size_t align>
  inline
  void Simple<SizeModificator, align>::deallocate( BaseAllocator::Pointer ptr, std::size_t) noexcept
  {
    stat_deallocate();
    stat_sys_deallocate();
    trace_message("Generics::Simple::deallocate(): ", reinterpret_cast<size_t>(ptr));
    delete[] static_cast<unsigned char*>(ptr);
  }

  template<typename SizeModificator, size_t align>
  inline void Simple<SizeModificator, align>::calc_sys_deallocate() noexcept
  {
  }
  ...
  template<typename SizeModificator, size_t align>
  inline PoolListSimple<SizeModificator, align>::PoolListSimple() noexcept
    : blocks_in_pool_(0)
  {
  }
  ...
  inline
  BaseAllocator::Pointer
  PoolListSimple<SizeModificator, align>::allocate( std::size_t& n) /*throw (MemoryOut)*/
  {
...

    {
      ...
      while (it != pool_memory_blocks_.end())
      {
        if (it->first >= to_alloc)
        {
          ...
          used_memory_blocks_.splice( used_memory_blocks_.begin(), pool_memory_blocks_, it);
          return ptr;
        }
        ++it;
      }
    ...
    BaseAllocator::Pointer ptr, std::size_t) noexcept
  {
    ...
    {
      *it = save;
      pool_memory_blocks_.splice( pool_memory_blocks_.begin(), used_memory_blocks_, it);
      return;
    }
  ...
  template<typename SizeModificator, size_t align>
  void PoolListSimple<SizeModificator, align>::calc_sys_deallocate() noexcept
  {
  }
  ...
  inline PoolListSimple<SizeModificator, align>::~PoolListSimple() noexcept
  {
    std::for_each(pool_memory_blocks_.begin(), pool_memory_blocks_.end(), get_eraser_());
...
//      assert(used_memory_blocks_.size() == 0);
  ...
  inline
  BaseAllocator::Pointer
  PoolMultiMapSimple<SizeModificator, align>::allocate( std::size_t& n) /*throw (MemoryOut)*/
  {
    SizeModificator::modify(n, align, true);
    std::size_t to_alloc = n;
    n -= sizeof(std::size_t);
  ...
  template<typename SizeModificator, size_t align>
  void PoolMultiMapSimple<SizeModificator, align>::calc_sys_deallocate() noexcept
  {
  ...
  template<typename SizeModificator, size_t align>
  inline PoolMultiMapSimple<SizeModificator, align>::~PoolMultiMapSimple() noexcept
  {
    std::for_each(pool_memory_blocks_.begin(), pool_memory_blocks_.end(), SizeBlockEraser());
  }
  ...
  inline BlocksPool::BlocksPool() noexcept
    : blocks_in_pool_(0)
  {
  }

  inline BaseAllocator::Pointer BlocksPool::allocate( std::size_t& n) /*throw (MemoryOut)*/
  {
    {
      Sync::PosixGuard lock(mutex_);
      stat_allocate();
  ...
  inline void BlocksPool::deallocate( BaseAllocator::Pointer ptr, std::size_t) noexcept
  {
    MemBlocks::value_type save(
      *(static_cast<std::size_t*>(ptr) - 1),
      ptr);
  ...
  void BlocksPool::calc_sys_deallocate() noexcept
  {
  }
  ...
  inline
  BaseAllocator::Pointer
  PoolMultiThread<SizeModificator>::allocate( std::size_t& n) /*throw (MemoryOut)*/
  {
    stat_allocate();
  ...
  inline
  void
  PoolMultiThread<SizeModificator>::deallocate( BaseAllocator::Pointer ptr, std::size_t) noexcept
  {
    stat_deallocate();
  ...
  template<typename SizeModificator>
  void PoolMultiThread<SizeModificator>::calc_sys_deallocate() noexcept
  {
    std::size_t amount_allocate = 0;
    std::size_t amount = 0;
    for (std::size_t i = 0; i < sizeof(allocators_) / sizeof(allocators_[0]); ++i)
    {
