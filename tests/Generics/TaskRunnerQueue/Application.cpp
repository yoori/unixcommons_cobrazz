...
void TestTask1s::execute() noexcept
{
  sleep(1);
    ...
    for (std::size_t j = 0; j < sizeof(TEST_PARAMS) / sizeof(TEST_PARAMS[0]); ++j)
    {
      ...
      for ( std::size_t i = 0; i < TASKS_AMOUNT; ++i)
      {
        try
        {
          ...
          task_runner_->enqueue_task(Generics::Task_var(new TestTask1s), &NEXT_TIME);
        ...
        catch (const Generics::TaskRunner::Overflow& e)
        {
          ...
          (NOW + Generics::Time(0,100000) < NEXT_TIME ?
            ...
            << NEXT_TIME - NOW << ", overflows amount=" << overflows_counter + 1 << std::endl
            << "NEXT_TIME=" << NEXT_TIME << ", NOW=" << NOW << std::endl
            : std::cout) << " Phase " << j << ": overflowed " << i << " " << e.what() << std::endl;
          ++overflows_counter;
        }
      }
      std::cout << "Phase " << j << ": " << overflows_counter << " overflows." << std::endl;
      if (TEST_PARAMS[j].PLANNED_OVERFLOW != overflows_counter)
      {
        std::cerr << "FAIL: Incorrect occurred overflows number " << overflows_counter
          << ", expected " << TEST_PARAMS[j].PLANNED_OVERFLOW <<
          "TaskRunner threads=" << TEST_PARAMS[j].THREADS_AMOUNT
          << " queue limit=" << TEST_PARAMS[j].QUEUE_LIMIT << std::endl << std::endl;
      }
      ...
      for ( std::size_t i = 0; i < TASKS_AMOUNT; ++i)
      {
        try
        {
          task_runner_->enqueue_task(Generics::Task_var(new TestTask1s), &TINFINITY);
          std::cout << "enqueued " << i << std::endl;
        }
        catch (const Generics::TaskRunner::Overflow& e)
        {
      ...
      {
        std::cerr << "Infinity Phase " << j << " FAIL: were " << overflows_counter
          << " overflows. " << "But overflow impossible when infinity awaiting queue."
...
void TestTasker::do_release_queue_test() /*throw (eh::Exception)*/
{
  ...
  (timer.elapsed_time() < Generics::Time(0,900000) ||
    ...
    << "Release waiting for " << timer.elapsed_time() << std::endl;
}
