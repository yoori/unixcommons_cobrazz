  ...
  {
    {
      ...
      std::cout << "read size of block #2:" << read_second_block->size() << std::endl;

      std::cout << "first symbol: '" << *(const char*)read_second_block->read_content()
                << "'" << std::endl;
    }

    {
...

      {
        ...
        std::cout << "first symbol: '" << *(const char*)first_block->read_content()
                  << "'" << std::endl;

        std::cout << "read size of block (" << second_index << "):"
                  << second_block->size() << std::endl;

        std::cout << "first symbol: '" << *(const char*)second_block->read_content()
                  << "'" << std::endl;
      }
    }
