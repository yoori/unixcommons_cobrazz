...
std::string profiler_hpp = "#ifndef _PROFILER_H\n"
                           "#define _PROFILER_H\n"
                           "#include <time.h>\n"
                           "struct _funcprof\n"
                           "{\n"
...
using FileLinesArray = std::vector<std::string>;
...
int func_number = 1;
int num_nspace = 0, num_class = 0, num_enum = 0, num_equal = 0, num_struct = 0, num_elements = 0,
    num_extern = 0;
...
int main(int argc, char* argv[])
{
  ...
  if (argc != 1)
  {
    if (!strncmp(argv[1], "help", 4))
    {
      ...
      std::cout << "1. mask=<extension 1>,<extension 2>,...,<extension n> <Directory 1> <Directory "
                   "2> ... <Directory N>" << std::endl;
      std::cout << "Example:" << std::endl;
      std::cout << "./Parser mask=cpp,hpp projects/Ad/Server2/ChannelSvcs/ChannelManager "
                   "projects/UnixCommons/src/Generics" << std::endl;
      std::cout << "It implements calls to profiler services into *.cpp and *.hpp files in "
                   "projects/Ad/Server2/ChannelSvcs/ChannelManager and "
                   "projects/UnixCommons/src/Generics directories." << std::endl;
      std::cout << "In addition, it creates a file 'funclist' with names of profiled functions in "
      ...
      std::cout << "It creates a file 'Func_5.log' which contains some information about calls of "
                   "the function with number 5 in the current directory." << std::endl;
      std::cout << "IMPORTANT: File 'funclist' must be in the current directory." << std::endl;

      std::cout
        << "3. clean=<extension 1>,<extension 2>,...,<extension n> <Directory 1> <Directory "
           "2> ... <Directory N>" << std::endl;
      std::cout << "Example:" << std::endl;
      std::cout << "./Parser clean=cpp,hpp projects/Ad/Server2/ChannelSvcs/ChannelManager "
                   "projects/UnixCommons/src/Generics" << std::endl;
      ...
      if (!strncmp(argv[1], "func=", 5) || !strncmp(argv[1], "main", 4))
    {
      ...
      FILE* log_list;
      using int_array = std::vector<unsigned int>;
      ...
      if (!strncmp(argv[1], "main", 4))
      {
        ...
        std::ignore = fread(plog_list, PROF_FUNCTIONS, sizeof(_funcprof), log_list);
      ...
      if (log_list != 0)
      {
        ...
        while (ifunc_list && !ifunc_list.eof())
        {
          ...
          func_name.push_back(
            std::string(temp_file_line, is - temp_file_line.begin(), std::string::npos));
        }
        ...
        for (unsigned int n = 0; n < main_indexes_array.size(); n++)
        {
          ...
          dot_out << "digraph FuncLog {" << std::endl << "rankdir = LR;" << std::endl
          ...
          while (bf)
          {
            ...
            std::ignore = fread(&FuncProf, 1, sizeof(_funcprof), log_list);
...
void SaveFunctionLog(unsigned int func_number, std::ofstream* _log_out, std::ofstream* _dot_out)
{
  ...
  double own_time_sec = double(FuncProf.tm.tv_sec - FuncProf.child_tm.tv_sec) +
  ...
  if (cf)
  {
    ...
    *_dot_out << function_number << "->" << func_number << "[label = \x22" << func_calls
              << " calls\x22];" << std::endl;
  ...
  *_log_out << "Index    |   " << "Calls    |   " << "Seconds  |   " << "Microseconds |  "
            << "Average time in seconds/microseconds" << std::endl;
  ...
  *_dot_out << func_number
            << "[shape = rectangle, style = filled, fillcolor = lightgrey, label = \x22"
            << func_name[func_number].substr(0, func_name[func_number].find("(")) << "\x5c\x6e"
            << temp_func_calls << " calls, " << time_buff << " seconds"
            << "\x5c\x6e" << "own time " << own_time_buff << "seconds" << "\x22];" << std::endl;
}
...
void ParseFiles(const char* current_dir_name)
{
  ...
  if (cur_dir)
  {
    ...
    while ((direntry = readdir(cur_dir)) != 0)
    {
      ...
      if ((next_dir_name != ".") && (next_dir_name != "..") && (next_dir_name != "Profiler.cpp") &&
          (next_dir_name != "Parser.cpp") && (next_dir_name != "Profiler.hpp") &&
          (next_dir_name != "Parser.hpp"))
      {
        ...
        else
        {
          ...
          {
            if (full_file_name.substr(full_file_name.length() - file_mask[ii].length()) ==
                file_mask[ii])
            {
...
void UpdateFile(const char* file_name)
{
  ...
  if (fp && fp.is_open())
  {
    while (fp && !fp.eof())
    {
      ...
      if (temp_file_line.substr(0, init_str.length() - 1) ==
          init_str.substr(1, init_str.length() - 1)) // Profiling Prof...
      {
        temp_file_line.erase();
        next_prof_str = false;
      }
      else if (next_prof_str)
      {
        fp_out << std::endl;
      }
      else
      ...
      if ((!clean_flag) && (temp_file_line.substr(0, 9) == "#include ") && (!inc_upd) &&
          (!old_profiler_hpp))
      {
        inc_upd = true;
        fp_out << profiler_hpp;
...
void SearchText(std::string* __pfile_line, std::string __mytext, int* __numtext)
{
  ...
  if (pos != std::string::npos) // __mytext SomeName
  {
    ...
    {
      if (((pos > temp_pos[2 * _i]) && (pos < temp_pos[((2 * _i) + 1)])) ||
          (temp_pos[((2 * _i) + 1)] == std::string::npos))
      {
        __one_mark = true;
      }
      ++_i;
    ...
    if (!__one_mark)
    {
      ...
      if (((brack_pos != std::string::npos) && (pos > brack_pos)) &&
          ((brack1_pos == std::string::npos) ||
            ((brack1_pos != std::string::npos) && (pos < brack1_pos))))
      {
        *__numtext = -1;
      }
      ++*__numtext;
  ...
  if ((__pfile_line->find(";") != std::string::npos) &&
      !((__pfile_line->find("{") != std::string::npos) &&
        (__pfile_line->find("}") != std::string::npos)))
  {
    *__numtext = 0;
  }
...
void SearchBrace(std::string* __pfile_line)
{
  ...
  if (pos != std::string::npos)
  {
    ...
    if (!_one_mark)
    {
      if ((num_extern != 0) || (num_nspace != 0) || (num_class != 0) || (num_enum != 0) ||
          (num_struct != 0)) // __mytext SomeName %s\n {
      {
        if (num_class != 0)
        {
      ...
      else if ((braces_num == 0) && (num_equal == 0))
      {
        num_elements = 0;
        ++braces_num;
        line_upd = true;
