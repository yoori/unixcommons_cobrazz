...
inline void Profiling::add_time(timespec& tm) noexcept
{
  timespec t = {tm.tv_sec + tm2.tv_sec - tm1.tv_sec, tm.tv_nsec + tm2.tv_nsec - tm1.tv_nsec};
  if (t.tv_nsec < 0)
  {
    t.tv_sec--;
...
void Profiling::SaveLog()
{
  ...
  if (stat_handle)
  {
    ...
    log_file = log_file.substr(log_file.find(" ") + 2,
                 log_file.find(" ", log_file.find(" ") + 1) - 3 - log_file.find(" ")) +
               ".log";
  }
