...
namespace Logging::Syslog
{
  namespace Helper
  {
    ...
    public:
       ...
       */
      explicit Connection(Config&& config) /*throw (eh::Exception)*/;
    ...
    using Connection_var = ReferenceCounting::FixedPtr<Connection>;
    ...
    public:
       ...
       * @param config configuration
       */
      explicit Handler(Config&& config) /*throw (eh::Exception)*/;

      /**
       * Writes record into the system log
       * @param record Specifies log record to publish.
       */
      virtual void publish(const LogRecord& record) /*throw (Exception, eh::Exception)*/;
...
namespace Logging::Syslog
{
  namespace Helper
  {
    ...
    inline Handler::Handler(Config&& config) /*throw (eh::Exception)*/
      : formatter_(config.formatter ? std::move(config.formatter) :
          Formatter_var( new Simple::Formatter(false, false, true, true)),
