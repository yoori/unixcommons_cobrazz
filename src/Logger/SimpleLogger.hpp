...
namespace Logging
{
  ...
  public:
     ...
     */
    explicit DerivedLogger(Config&& config) /*throw (eh::Exception)*/;

  protected:
    /**
     * Destructor
     */
    virtual ~DerivedLogger() noexcept;
  };
}
...
namespace Logging::Simple
{
  ...
  inline Logger::Logger(Handler* handler, Config&& config)
    /*throw (eh::Exception)*/
  ...
  inline void Logger::log_level(unsigned long value) noexcept
  {
...
namespace Logging
{
  ...
  DerivedLogger<Config, Handler>::DerivedLogger(Config&& config)
    /*throw (eh::Exception)*/
    : Simple::Logger( Handler_var(new Handler(std::move(config))), std::move(config))
  {
  }
