...
namespace Logging::Process
{
  namespace Helper
  {
    ...
    private:
       ...
       */
      void make_sockets_(int* read_descriptor, int* write_descriptor)
        /*throw (Exception, eh::Exception)*/;
       ...
       */
      bool make_fork_() /*throw (Exception, eh::Exception)*/;

      /**
       * Creates pipe and forks process
       * @return true, if a process is a child, false if parent
       */
      bool init_() /*throw (Exception, eh::Exception)*/;
...
namespace Logging::Process
{
  ...
  Config::Config(const char* path, char* argv[], char* envp[],
    const Formatter* formatter, bool wait_for_child,
    size_t preallocated_size) /*throw (eh::Exception)*/
    : Helper::Config(formatter, path, argv, envp, wait_for_child, preallocated_size)
  {
  }
}
