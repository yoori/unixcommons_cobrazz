...
namespace Logging::File
{
  namespace Helper
  {
        ...
        formatter_(config.formatter, config.preallocated_size)
    {
      ...
      String::StringManip::strlcpy(file_name_, config.file_name.c_str(), sizeof(file_name_));
      ...
      {
        String::StringManip::strlcpy(cur_file_name_, file_name_, sizeof(cur_file_name_));
      }

      if (::stat(cur_file_name_, &file_stat_) == 0 && file_stat_.st_size > 0)
      {
      ...
      if (!outfile_)
      {
        eh::throw_errno_exception<Exception>(FNE, "failed to open file '", cur_file_name_, "'");
      }
    }

    bool Handler::rotate_if_required(const Generics::ExtendedTime& time)
      /*throw (Exception, eh::Exception)*/
    {
      if (!outfile_ || ::stat(cur_file_name_, &file_stat_) != 0)
      {
        ...
        {
          eh::throw_errno_exception<Exception>(FNE, "failed to stat file '", cur_file_name_, "'");
        }
      }

      for (Policies::PolicyList::iterator it = POLICIES_.begin(); it != POLICIES_.end(); ++it)
      {
      ...
      /*throw (Exception, eh::Exception)*/
    {
      ...
      if (std::fputs(line.get(), outfile_) == EOF || std::fflush(outfile_) == EOF ||
        ::stat(cur_file_name_, &file_stat_) != 0)
      {
        ...
        if (!outfile_)
        {
          eh::throw_errno_exception<Exception>(FNE, "failed to reopen file '", cur_file_name_, "'");
        }
    ...
    void Handler::rotate(const Generics::ExtendedTime& time)
    /*throw (Exception, eh::Exception)*/
    {
      ...
      FileName new_name;
      {
        ...
        if (EXTENDED_NAME_FORMAT_)
        {
          ...
          ostr << file_name_ << "_" << ORDER_NUM_ << "." <<
            FROM_NUM_ << "_" << time.format("%Y%m%d.%H%M%S%q") << "." << RND;
        }
        else
        {
  ...
  namespace Policies
  {
    ...
    bool TimeSpanPolicy::need_rotation(const Helper::Handler& file_handler)
      /*throw (Exception, eh::Exception)*/
    {
      ...
      return file_handler.log_time() >= file_handler.log_create_time() + rotation_time;
    }
    ...
    bool SizeSpanPolicy::need_rotation(const Helper::Handler& file_handler)
      /*throw (Exception, eh::Exception)*/
    {
    ...
    bool AlignedTimeSpanPolicy::need_rotation( const Helper::Handler& file_handler)
      /*throw (Exception, eh::Exception)*/
    {
      if (file_handler.file_stat().st_size == 0)
      {
        ...
        file_handler.log_create_time() != last_create_time_)
      {
        ...
        Generics::ExtendedTime cur(log_create_time.get_time( file_handler.get_time_zone()));
