  ...
  namespace Helper
  {
        ...
        WAIT_FOR_CHILD_(config.wait_for_child)
    {
      if (init_())
      {
        if (!config.argv)
        {
          const char* argv[4] = { "sh", "-c", config.command_path.c_str(), 0 };
    ...
    void Handler::make_sockets_(int* read_descriptor, int* write_descriptor)
      /*throw (Exception, eh::Exception)*/
    {
      int pipe_descriptors[2];
      if (pipe(pipe_descriptors) == -1)
      {
        eh::throw_errno_exception<Exception>(FNE, "failed to create sockets");
      }
    ...
    bool Handler::make_fork_() /*throw (Exception, eh::Exception)*/
    {
      switch (child_ = fork())
      {
      case -1:
      ...
      eh::throw_errno_exception<Exception>(FNE, "failed to fork");
      return false; //Never executed
    }

    bool Handler::init_() /*throw (Exception, eh::Exception)*/
    {
      int reader, writer;
      bool child;
