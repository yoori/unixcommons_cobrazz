...
namespace
{
  ...
  public:
    ...
    void advance(size_t length) noexcept;
  ...
  inline Buffer::Buffer(char* buf, size_t length) /*throw (eh::Exception)*/
  ...
  inline char* Buffer::get() const noexcept
  {
  ...
  inline void Buffer::advance(size_t length) noexcept
  {
    if (size_ > 0)
    {
      buff_ptr_ += length;
  ...
  inline void Buffer::advance() noexcept
  {
    advance(size_ > 0 ? strlen(buff_ptr_) : 0);
  }
};

namespace Logging::Simple
{
  //
  // Logger class
  //
  ...
  size_t Formatter::required_size(const LogRecord& record) const
  ...
  bool Formatter::format(const LogRecord& record, char* buf, size_t size) const
    /*throw (Exception, eh::Exception)*/
  {
    {
    ...
    if (log_time_)
    {
      const Generics::ExtendedTime& record_time( record.time.get_time(record.time_zone));
    ...
    if (log_severity_)
    {
      const size_t SEVERITIES = sizeof(SEVERITY_LABLES) / sizeof(*SEVERITY_LABLES);

      const String::SubString& SEVERITY = SEVERITY_LABLES[record.severity < SEVERITIES ?
    ...
    if (log_time_ || log_severity_ || log_aspect_ || log_thread_id_ || log_process_id_)
    {
      assert(buff.size() >= 2);
      buff.get()[0] = ':';
      buff.get()[1] = ' ';
...
namespace Logging
{
  ...
  Formatter_var FormatWrapper::create_default_formatter_() /*throw (eh::Exception)*/
  {
