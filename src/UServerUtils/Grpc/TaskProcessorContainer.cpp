  ...
  : logger_(logger)
{
  ...
  thread_pool_config.threads = event_thread_pool_config.threads;
  thread_pool_config.thread_name = event_thread_pool_config.thread_name;
  thread_pool_config.ev_default_loop_disabled = event_thread_pool_config.ev_default_loop_disabled;
  thread_pool_config.defer_events = event_thread_pool_config.defer_events;

  task_processor_pools_ =
    TaskProcessorPoolsPtr(
...
TaskProcessorContainer::TaskProcessorPtr
TaskProcessorContainer::add_task_processor(
  const TaskProcessorConfig& config)
{
  return add_task_processor_helper(config, false);
}

TaskProcessorContainer::TaskProcessorPtr
TaskProcessorContainer::get_task_processor(
...
TaskProcessorContainer::TaskProcessorPtr
TaskProcessorContainer::get_main_task_processor()
{
  return get_task_processor(main_task_processor_name_);
}

void TaskProcessorContainer::stop() noexcept
{
...
TaskProcessorContainer::TaskProcessorPtr
TaskProcessorContainer::add_task_processor_helper(
  const TaskProcessorConfig& config,
  const bool is_main)
{
