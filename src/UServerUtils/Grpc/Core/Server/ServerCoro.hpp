...
namespace UServerUtils::Grpc::Core::Server
{
  ...
  public:
    ServerCoro(const ConfigCoro& config, Logging::Logger* logger);
      ...
      const std::optional<std::size_t> number_coro = {})
    {
      ...
      {
        Stream::Error stream;
        stream << FNS << ": state not equal AS_NOT_ACTIVE";
        throw Exception(stream);
      }

      server_->register_handler<typename Service::Handler>();

      constexpr auto rpc_type = Service::Handler::Traits::rpc_type;
      constexpr const char* method_name = Service::Handler::Traits::method_name();
