...
namespace UServerUtils::Grpc::Core::Server
{
  ...
  public:
    ...
    auto get_producer(const MethodName method_name)
    {
      ...
      {
        Stream::Error stream;
        stream << FNS << ": not existing service for name=" << method_name;
      ...
      auto producer = std::make_unique<Producer>(queue->GetProducer());

      ReaderPtr reader(new Reader(logger_.in(), queue->GetConsumer()));
      ...
      const std::optional<std::size_t> number_coro)
    {
      ...
      {
	Stream::Error stream;
        stream << FNS << ": state must be AS_NOT_ACTIVE";
        throw Exception(stream);
      }
      ...
      {
        Stream::Error stream;
        stream << FNS << ": service with method_name=" << method_name << " already exist";
        throw Exception(stream);
      }
      ...
      if (number_coro)
      {
        ...
        {
          Stream::Error stream;
          stream << FNS << ": Queue::Create return null ptr";
          throw Exception(stream);
        }

        producers_.emplace_back(std::make_shared<Producer>(queue->GetProducer()));
        ...
        for (std::size_t i = 1; i <= *number_coro; ++i)
        {
          worker_tasks_.emplace_back(
            add_coroutine(
              task_processor,
              ReaderPtr(new Reader(logger_, queue->GetConsumer())),
              service,
              false,
              rpc_type));
        }
