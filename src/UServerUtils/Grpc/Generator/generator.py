...
class _CodeGenerator:
    ...
    def _generate_code_with_service(self) -> None:
        ...
        for file_type, file_ext in self._iter_src_files():
            for provider in ['usrv', 'cobrazz']:
                template_name = f'{file_type}.{provider}.{file_ext}.jinja'
                template = self.jinja_env.get_template(template_name)

                file = self.response.file.add()
                file.name = self._proto_file_dest(
                    file_type=file_type, file_ext=file_ext, provider=provider
                )
    ...
    def _generate_code_empty(self) -> None:
        for file_type, file_ext in self._iter_src_files():
            file = self.response.file.add()
            file.name = self._proto_file_dest(
                file_type=file_type, file_ext=file_ext, provider='usrv'
            )
    ...
    def _proto_file_dest(self, file_type: str, file_ext: str, provider: str) -> str:
        return f'{self._proto_file_stem()}_{file_type}.{provider}.pb.{file_ext}'
