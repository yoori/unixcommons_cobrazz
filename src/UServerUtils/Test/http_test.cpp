...
public:
  TestHandler(
    ...
    const UServerUtils::Http::Server::HandlerConfig& handler_config,
...
public:
    ...
    const bool enable_statistics_test)
  {
    ...
    auto init_func = [logger = logger_, unix_socket_path, response_body_stream, statistics_provider] (
      ...
      UServerUtils::ComponentsBuilder::StatisticsProviderInfo statistics_provider_info;
      ...
      auto components_builder = std::make_unique<UServerUtils::ComponentsBuilder>(std::move(statistics_provider_info));
      auto& statistic_storage = components_builder->get_statistics_storage();

      UServerUtils::Http::Server::ServerConfig server_config;
      ...
      UServerUtils::Http::Server::HandlerConfig handler_config;
    ...
    UServerUtils::Http::Client::ClientConfig client_config;
    client_config.io_threads = 8;
    UServerUtils::Http::Client::Client client(client_config, task_processor);

    auto request = client.create_request().retry(10).timeout(1000);
    if (method == Method::GET)
    {
      request.method(UServerUtils::Http::Client::HttpMethod::kGet);
    }
    else if (method == Method::POST)
    {
      request.method(UServerUtils::Http::Client::HttpMethod::kPost);
    }
  ...
  void do_statistic_test(
    UServerUtils::Http::Client::Client& client,
    const std::shared_ptr<TestStatisticsProvider>& test_statistics_provider)
  {
    ...
    auto request_monitor = client.create_request()
      .retry(10)
      .timeout(1000)
      .method(UServerUtils::Http::Client::HttpMethod::kGet)
