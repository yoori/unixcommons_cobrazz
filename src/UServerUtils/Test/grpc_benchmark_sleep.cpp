...
public:
  ...
  void chat(chatCall& call) override
  {
    ...
    while (call.Read(request))
    {
      ...
      {
        userver::engine::SleepFor(std::chrono::milliseconds(sleep_duration_));
      }
...
public:
  ...
  bool active() override
  {
    ...
    return state != State::kInvalid && state != State::kCompleted;
  }
...
public:
  Application(
    ...
    const std::size_t sleep_duration)
    : port_(port),
  ...
  int run()
  {
    ...
    coro_pool_config.initial_size = 5000;
    ...
    auto inititialize_func = [
      ...
      UServerUtils::UServerGrpc::GrpcServerBuilderPtr server_builder =
        std::make_unique<UServerUtils::UServerGrpc::GrpcServerBuilder>(
          ...
          statistic_storage);
      UServerUtils::Grpc::Server::GrpcServiceBase_var service(new Service(sleep_duration_));
...
int main(int /*argc*/, char** /*argv*/)
{
  try
  {
    ...
    const std::size_t number_client = 20000;
    const std::string message = "hello";
    const std::size_t number_initial_message = 1;
    ...
    const std::size_t sleep_duration = 0;  // <--!ms
