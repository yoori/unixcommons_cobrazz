...
namespace String::Utf8Set
{
  ...
  using Utf8Chars = Generics::CompressedSet<Utf8Char>;
   ...
   */
  Utf8Char get_char(const char* symbol, unsigned long* poctets = 0)
    /*throw (eh::Exception)*/;
...
namespace String
{
  ...
  public:
     ...
     */
    explicit Utf8Category(const char* symbols, bool check_zero = false)
     ...
     */
    explicit Utf8Category(const Utf8Set::Utf8Chars& chars) /*throw (eh::Exception)*/;

    /**
     * Constructor
     * @param tree static definitions of tree stored UTF8 category
     */
    explicit constexpr Utf8Category(UnicodeProperty::TreeStartNode& tree)
      /*throw (eh::Exception)*/;
     ...
     */
    void swap(Utf8Category& category) noexcept;
     ...
     */
    bool is_owned(const char* str) const noexcept;
     ...
     * pseudo-UTF-8 symbol
     */
    const char* find_owned(const char* str, unsigned long* octets = 0) const noexcept;
     ...
     * pseudo-UTF-8 symbol
     */
    const char* find_nonowned(const char* str, unsigned long* octets = 0) const noexcept;
  ...
  protected:
     ...
     * Utf8Category.
     */
    const UnicodeProperty::TreeStartNode& get_container_() const noexcept;

  private:
    void clear_() noexcept;

    void clear_(const UnicodeProperty::Node* node, unsigned long depth) noexcept;

    void init_(const Utf8Set::Utf8Chars& chars) /*throw (eh::Exception)*/;
...
namespace String
{
  inline constexpr Utf8Category::Utf8Category(UnicodeProperty::TreeStartNode& tree)
    /*throw (eh::Exception)*/
    : nodes_(tree), need_cleaning_(false)
  {
  }

  inline bool Utf8Category::is_owned(const char* str) const noexcept
  {
    return UnicodeProperty::belong(get_container_(), str);
  }

  inline bool Utf8Category::operator ()(const char* str) const noexcept
  {
    return is_owned(str);
  }

  inline const UnicodeProperty::TreeStartNode& Utf8Category::get_container_() const noexcept
  {
    return nodes_;
  }
}
