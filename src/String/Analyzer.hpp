...
namespace String::SequenceAnalyzer
{
  ...
  public:
     ...
     */
    Analyzer(const AnalyzerParams& params, Generics::ActiveObjectCallback* callback)
    ...
    void process_char_sequence(std::istream& istr, ResultType& result_arg)
      /*throw (Exception, eh::Exception)*/;

  private:
    using ChSeq = std::list<char>;

    struct TreeNode;
    using TreeNode_var = ReferenceCounting::QualPtr<TreeNode>;
    using ConstTreeNode_var = ReferenceCounting::QualPtr<const TreeNode>;
    struct TreeNode : public ReferenceCounting::DefaultImpl<>
    {
    public:
      TreeNode() /*throw (eh::Exception)*/;

      using Children = ReferenceCounting::Deque<TreeNode_var>;
    ...
    TreeNode_var process_char_subsequence(std::istream& istr, TreeNode_var start_general_node)
      /*throw (Exception, NoncriticalException, eh::Exception)*/;

    void set_node_repeat_amount(std::istream& istr, TreeNode_var repeat_node)
      /*throw (Exception, NoncriticalException, eh::Exception)*/;

    template <typename ResultType>
    void process_range_sequence(std::istream& istr, ResultType& result_arg)
      /*throw (Exception, eh::Exception)*/;
    ...
    static bool check_symbol_in_set(const CharSet& chars, char ch)
      /*throw (eh::Exception)*/;

    void recognize_symbol(std::istream& istr)
      /*throw (Exception, NoncriticalException, eh::Exception)*/;

    void recognize_range_symbol(std::istream& istr)
      /*throw (Exception, NoncriticalException, eh::Exception)*/;
    ...
    bool not_padding_symb(char ch) const
      /*throw (eh::Exception)*/;

    static bool not_digit(char ch) noexcept;

    bool not_allowed_char_range(char ch) const
      /*throw (eh::Exception)*/;

    bool not_allowed_str_range(char ch) const
      /*throw (eh::Exception)*/;

    bool check_allowed_int_range(unsigned int number) const
      /*throw (eh::Exception)*/;

    static unsigned int extract_number(const ChSeq& chars, bool& num_range)
      /*throw (Exception, NoncriticalException, eh::Exception)*/;
    ...
    template <typename ResultType>
    void interprete_and_flush_tree(ResultType& result_arg) const
      /*throw (Exception, eh::Exception)*/;
    ...
    void
    write_result_arg(std::set<Element, Compare, Allocator>& main_set, Stream::Dynamic& my_str) const
      /*throw (Exception, eh::Exception)*/;
  ...
  private:
     ...
     * to put empty lexeme
     */
    bool pass_separator_symbols_(std::istream& istr) /*throw (eh::Exception)*/;

    void create_tree_() /*throw (eh::Exception)*/;

    /**
     * Simple create node and add it to tree.
     * @param start_general_node tree to stick node.
     */
    void create_node_(TreeNode_var& start_general_node)
      /*throw (Exception)*/;
     ...
     */
    TreeNode_var create_tree_node_(TreeNode_var& start_general_node)
     ...
     */
    void check_readability_(std::istream& istr, char stop_symbol) const
      /*throw (NoncriticalException)*/;

    void recognize_shielded_symbol_(std::istream& istr)
      /*throw (Exception, NoncriticalException, eh::Exception)*/;
...
namespace String::SequenceAnalyzer
{
  inline bool Analyzer::check_symbol_in_set(const CharSet& ch_set, char ch)
    /*throw (eh::Exception)*/
  {
    return ch_set(ch);
  }

  inline bool Analyzer::not_padding_symb(char ch) const
    /*throw (eh::Exception)*/
  {
    return ch != init_params_.padding_symb;
  }

  inline bool Analyzer::not_digit(char ch) noexcept
  {
    return !String::AsciiStringManip::NUMBER(ch);
  }

  inline bool Analyzer::not_allowed_char_range(char ch) const
    /*throw (eh::Exception)*/
  {
    return !check_symbol_in_set(init_params_.char_range_bounds, ch);
  }

  inline bool Analyzer::not_allowed_str_range(char ch) const
    /*throw (eh::Exception)*/
  {
    return !check_symbol_in_set(init_params_.str_char_range_bounds, ch);
  }

  inline bool Analyzer::check_allowed_int_range(unsigned int number) const
    /*throw (eh::Exception)*/
  {
    ...
    /*throw (Exception, eh::Exception)*/
  {
    for (unsigned short int i = 0; i < cur_node->repeat_amount; i++)
    {
      ...
      {
        for (TreeNode::Children::const_iterator iter = cur_node->child_list.begin();
  ...
  template <typename ResultType>
  void Analyzer::interprete_and_flush_tree(ResultType& result_arg) const
    /*throw (Exception, eh::Exception)*/
  {
    ...
    if (!init_params_.immediate_range_mode)
    {
      ...
      while (istr.good())
      {
        ...
        else
        {
          ...
          {
            Stream::Error ostr;
            ostr << FNS << "Got NoncriticalException while trying to process "
              "symbol. Description: " << e.what() << "\n"
              "Proceeding from the nearest separator";
