...
namespace
{
  void reserved_for_future_implementations(String::SubString function, const char* str)
  ...
  template <typename T>
  void debug(T /*str*/) /*throw (eh::Exception)*/
  {
    // std::cerr << x << std::endl
  }
}

namespace String::SequenceAnalyzer
{
  //
  ...
  inline Analyzer::TreeNode::TreeNode() /*throw (eh::Exception)*/
    : repeat_amount(1)
  {
    debug(FNB);
  }

  inline Analyzer::TreeNode::~TreeNode() noexcept
  {
    debug(FNB);
  }
  ...
  Analyzer::Analyzer(const AnalyzerParams& init_params, Generics::ActiveObjectCallback* callback)
      ...
      callback_(ReferenceCounting::add_ref(callback))
  {
    ...
    if (init_params_.allow_repeat)
    {
      ...
      {
        Stream::Error ostr;
        ostr << FNS << "Not defined symbols for marking number of lexeme retries.";
        throw Exception(ostr);
      }
      ...
      {
        Stream::Error ostr;
        ostr << FNS << "Not defined symbols to quote that a lexeme or a group of "
    ...
    else
    {
      ...
      {
        Stream::Error ostr;
        ostr << FNS << "Immediate range mode cannot be used with disallowed ranges.";
        throw Exception(ostr);
      }
    }
    ...
    {
      init_params_.recursion_max_depth = Generics::safe_next(init_params_.recursion_max_depth);
    }
    ...
    if (init_params_.use_int_range)
    {
      ...
      {
        Stream::Error ostr;
        ostr << FNS << "Empty list of bounds of allowed unsigned int ranges.";
        throw Exception(ostr);
      }
      ...
      {
        Stream::Error ostr;
        ostr << FNS << "default_int_range_start is not a number within one of "
    ...
    {
      reserved_for_future_implementations(FNB, "Currently use_char_range should be set to false.");
      ...
      {
        Stream::Error ostr;
        ostr << FNS << "default_char_range_start is not a char within one of "
    ...
    {
      reserved_for_future_implementations(FNB, "Currently use_str_range should be set to false.");

      if (init_params_.str_char_range_bounds.empty())
      {
        Stream::Error ostr;
        ostr << FNS << "Empty list of bounds of allowed chars for str ranges.";
        throw Exception(ostr);
      }
  ...
  bool Analyzer::pass_separator_symbols_(std::istream& istr)
    /*throw (eh::Exception)*/
  {
    if (init_params_.ignore_successive_separators)
    {
      while (istr.good() && (check_symbol_in_set(init_params_.main_separators,
          current_symbol_) || (init_params_.allow_ignored_symbs &&
    ...
    // ignore_successive_separators=false ,, -> ""
    if (istr.good() && check_symbol_in_set( init_params_.main_separators, current_symbol_))
    {
      istr.get(current_symbol_);
      if (istr.good() && check_symbol_in_set( init_params_.main_separators, current_symbol_))
      {
  ...
  void Analyzer::create_tree_() /*throw (eh::Exception)*/
  {
    try
    {
  ...
  void Analyzer::create_node_(TreeNode_var& start_general_node)
    /*throw (Exception)*/
  {
    try
    {
  ...
  Analyzer::TreeNode_var Analyzer::create_tree_node_(TreeNode_var& start_general_node)
  ...
  inline void Analyzer::check_readability_(std::istream& istr, char stop_symbol) const
    /*throw (NoncriticalException)*/
  {
    if (!istr.good() && current_symbol_ != stop_symbol)
    {
      Stream::Error ostr;
      ostr << FNS << "bad expression on input, missing closing symbol=" << stop_symbol;
      throw NoncriticalException(ostr);
    }
  }
  ...
  /**
   */
  void Analyzer::recognize_shielded_symbol_(std::istream& istr)
    /*throw (Exception, NoncriticalException, eh::Exception)*/
  {
    ...
    {
      ShieldMap::const_iterator it = init_params_.shield_map.find(current_symbol_);
    ...
    if (!init_params_.allow_ignored_symbs || !check_symbol_in_set(init_params_.ignored_symbs,
        current_symbol_))
    {
      short int code = current_symbol_;
      Stream::Error ostr;
      ostr << FNS << "illegal symbol '" << current_symbol_ << "' with code " << code;
      throw NoncriticalException(ostr);
    }
  }

  void Analyzer::recognize_symbol(std::istream& istr)
    /*throw (Exception, NoncriticalException, eh::Exception)*/
  {
    if (init_params_.allow_repeat && current_symbol_ == init_params_.retry_part_symb.first())
    {
      if (!current_node_->node_val.empty())
      {
        Stream::Error ostr;
        ostr << FNS << "Symbol '" << current_symbol_ << "' is not the first symbol in lexeme";
        throw NoncriticalException(ostr);
      }
      ...
      {
        Stream::Error ostr;
        ostr << FNS << "failed to read the next symbol after the retry part";
        throw Exception(ostr);
      }
      ...
      if (current_symbol_ != init_params_.num_retries_symb.first())
      {
        ...
        ostr << FNS << "The retry part should be followed by the '" <<
          init_params_.num_retries_symb.first() << "' symbol with repeat number";
        throw NoncriticalException(ostr);
      }
      set_node_repeat_amount(istr, general_node);
    }
    else if (init_params_.allow_range && current_symbol_ == init_params_.range_part_symb.first())
    {
      TreeNode_var general_node;
      general_node = current_node_;
    ...
    else if (init_params_.allow_repeat && current_symbol_ == init_params_.num_retries_symb.first())
    {
    ...
    else if (check_symbol_in_set(init_params_.regular_symbs, current_symbol_))
    {
      current_node_->node_val.push_back(current_symbol_);
    }
    else if (init_params_.allow_ignored_symbs) // unknown symbol = irregular
    {
      if (!check_symbol_in_set(init_params_.ignored_symbs, current_symbol_))
      {
        short int code = current_symbol_;
        Stream::Error ostr;
        ostr << FNS << "illegal symbol '" << current_symbol_ << "' with code " << code;
  ...
  Analyzer::process_char_subsequence(std::istream& istr, Analyzer::TreeNode_var start_general_node)
    /*throw (Exception, NoncriticalException, eh::Exception)*/
  {
    TreeNode_var final_general_node = create_tree_node_(start_general_node);
    recursion_depth_++;

    istr.get(current_symbol_);
    while (istr.good() && current_symbol_ != init_params_.retry_part_symb.second())
    {
      if (check_symbol_in_set(init_params_.main_separators, current_symbol_))
      {
  ...
  void Analyzer::set_node_repeat_amount(std::istream& istr, TreeNode_var repeat_node)
    /*throw (Exception, NoncriticalException, eh::Exception)*/
  {
    ...
    if (istr.good())
    {
      ...
      if (current_symbol_ != init_params_.num_retries_symb.second())
      {
        short int code  = current_symbol_;
        Stream::Error ostr;
        ostr << FNS << "the next symbol after lexeme repeat amount should be '" <<
    ...
    {
      Stream::Error ostr;
      ostr << FNS << "failed to read lexeme repeat amount after the '" <<
  ...
  void Analyzer::recognize_range_symbol(std::istream& istr)
    /*throw (Exception, NoncriticalException, eh::Exception)*/
  {
    if (init_params_.allow_repeat && (current_symbol_ == init_params_.retry_part_symb.first()))
    {
      ...
      istr.get(current_symbol_);
      if (!istr.good() || current_symbol_ != init_params_.num_retries_symb.first())
      {
        Stream::Error ostr;
        ostr << FNS << "The retry part close symbol is not followed with the "
    ...
    else if (check_symbol_in_set(init_params_.regular_range_symbs, current_symbol_))
    {
      current_node_->node_val.push_back(current_symbol_);
    }
    ...
    /*throw (Exception, NoncriticalException, eh::Exception)*/
  {
    ...
    {
      if (check_symbol_in_set(init_params_.range_separators, current_symbol_))
      {
        current_node_->child_list.push_back(final_general_node);
        if (current_node_->child_list.size() != 1)
        {
        ...
        if (init_params_.ignore_successive_separators)
        {
          while (istr.good() && (check_symbol_in_set(init_params_.range_separators,
              current_symbol_) || (init_params_.allow_ignored_symbs &&
                check_symbol_in_set(init_params_.ignored_symbs,
                  current_symbol_))))
          {
      ...
      {
        current_node_ = process_range_list(istr, true, range_seq_close_symbol, current_node_);
      }
      else
      {
  ...
  unsigned int Analyzer::extract_number(const ChSeq& ch_list, bool& num_range)
    /*throw (Exception, NoncriticalException, eh::Exception)*/
  {
    unsigned int extracted_int = 0;

    ChSeq::const_iterator it = std::find_if(ch_list.begin(), ch_list.end(), Analyzer::not_digit);
    if (it != ch_list.end())
    {
      num_range = false;
    }
    ...
    {
      for (ChSeq::const_iterator iter = ch_list.begin(); iter != ch_list.end(); ++iter)
      {
  ...
  Analyzer::process_range_list(std::istream& istr,
    ...
    /*throw (Exception, NoncriticalException, eh::Exception)*/
  {
    unsigned int range_part1_length_with_padding = start_general_node->node_val.size();
    ...
    istr.get(current_symbol_);
    while (istr.good() && !check_symbol_in_set(init_params_.range_separators, current_symbol_) &&
      !(use_range_seq_close_symbol && current_symbol_ == range_seq_close_symbol) &&
    ...
    if (range_part2_length_with_padding != range_part1_length_with_padding)
    {
      str_range = false;
      char_range = false;
    }
    ...
    if (range_part2_length == 0 || range_part2_length < range_part1_length)
    {
      Stream::Error ostr;
    ...
    if (range_part1_length_with_padding != range_part1_length &&
        numb_range && range_part1_length_with_padding != range_part2_length_with_padding)
    {
      numb_range = false;
    }
    ...
    if ((use_range_seq_close_symbol && (current_symbol_ != range_seq_close_symbol &&
          current_symbol_ == init_params_.range_part_symb.first())) ||
     (!use_range_seq_close_symbol && current_symbol_ == init_params_.range_part_symb.first()))
    {
      TreeNode_var general_node;
      general_node = current_node_;
      current_node_ = process_range_subsequence(istr,
    ...
    /*throw (Exception, NoncriticalException, eh::Exception)*/
  {
    ...
    for (unsigned int i = start_int; i <= final_int; i++)
    {
      ...
      current_node_->node_val.insert(current_node_->node_val.end(), tmpstr.begin(), tmpstr.end());
  ...
  bool
  Analyzer::check_param2_after_param1( const ChSeq& /*ch_list1*/, const ChSeq& /*ch_list2*/) const
    /*throw (eh::Exception)*/
  {
