...
namespace String::TextTemplate
{
  ...
  std::string Basic::StringItem::key(const ArgsCallback& /*callback*/) const
    /*throw (eh::Exception)*/
  {
    return std::string();
  }
  ...
  std::string Basic::VarItem::key(const ArgsCallback& callback) const
    /*throw (eh::Exception)*/
  {
    std::string result;
  ...
  Basic::Basic(const SubString& str, const SubString& start_lexeme, const SubString& end_lexeme)
  ...
  void
  Basic::init(const SubString& source, const SubString& start_lexeme, const SubString& end_lexeme)
    /*throw (InvalidTemplate, TextTemplException, eh::Exception)*/
  {
    items_.clear();
    fixed_size_ = 0;
    ...
    while (!str.empty())
    {
      ...
      {
        Stream::Error ostr;
        ostr << FNS << "invalid template: closing lexeme (" << end_lexeme <<
          ") not found. Template:\n'" << source << "'";
        throw InvalidTemplate(ostr);
      }

      items_.push_back(Item_var( new VarItem(str.substr(begin, end - begin))));

      str = str.substr(end + end_lexeme.length());
    }
  }

  std::string Basic::instantiate(const ArgsCallback& args) const
  ...
  void Basic::keys(const ArgsCallback& args, Keys& keys) const
    /*throw (UnknownName, TextTemplException, eh::Exception)*/
  {
    keys.clear();
    for (Items::const_iterator it = items_.begin(); it != items_.end(); ++it)
  ...
  void
  String::init(const SubString& str, const SubString& start_lexeme, const SubString& end_lexeme)
    /*throw (InvalidTemplate, TextTemplException, eh::Exception)*/
  {
  ...
  String::String(const SubString& str, const SubString& start_lexeme, const SubString& end_lexeme)
    /*throw (InvalidTemplate, TextTemplException, eh::Exception)*/
  {
    init(str, start_lexeme, end_lexeme);
  }
  ...
  void IStream::init(std::istream& istr, const SubString& start_lexeme, const SubString& end_lexeme)
    /*throw (InvalidTemplate, TextTemplException, eh::Exception)*/
  {
  ...
  IStream::IStream(std::istream& istr, const SubString& start_lexeme, const SubString& end_lexeme)
    /*throw (InvalidTemplate, TextTemplException, eh::Exception)*/
  {
  ...
  namespace
  {
     ...
     */
    void encode_utf8_(std::string&& value, std::string& encoded)
      /*throw (eh::Exception)*/
    {
     ...
     * @param encoded resulted encoded string
     */
    void encode_xml_(std::string&& value, std::string& encoded)
      /*throw (StringManip::InvalidFormatException, eh::Exception)*/
    {
      StringManip::xml_encode(value.c_str(), encoded);
    }

    void encode_mime_(std::string&& value, std::string& encoded)
      /*throw (StringManip::InvalidFormatException, eh::Exception)*/
    {
      StringManip::mime_url_encode(std::string_view(value), encoded);
    }

    void encode_js_unicode_(std::string&& value, std::string& encoded)
      /*throw (StringManip::InvalidFormatException, eh::Exception)*/
    {
      StringManip::js_unicode_encode(value.c_str(), encoded);
    }

    void encode_base64_(std::string&& value, std::string& encoded)
      /*throw (StringManip::InvalidFormatException, eh::Exception)*/
    {
      StringManip::base64mod_encode(encoded, value.data(), value.size());
    }

    void encode_js_(std::string&& value, std::string& encoded)
      /*throw (StringManip::InvalidFormatException, eh::Exception)*/
    {
    ...
    public:
       ...
       */
      Args::ValueEncoder get_value_encoder(const SubString& key) const noexcept;
       ...
       */
      void
      register_value_encoder(const char* key, Args::ValueEncoder encoder) /*throw (eh::Exception)*/;

    private:
      using RelationType = std::map<const SubString, Args::ValueEncoder>;

      RelationType relation_;
    };

    inline Args::ValueEncoder EncoderHolder::get_value_encoder(const SubString& key) const noexcept
    {
   ...
   * Default encodings
   */
  const ArgsEncoder::EncoderItem ArgsEncoder::EI_UTF8("utf8", encode_utf8_);
  const ArgsEncoder::EncoderItem ArgsEncoder::EI_MIME_URL("mime-url", encode_mime_);
  const ArgsEncoder::EncoderItem ArgsEncoder::EI_XML("xml", encode_xml_);
  const ArgsEncoder::EncoderItem ArgsEncoder::EI_JS_UNICODE("js-unicode", encode_js_unicode_);
  const ArgsEncoder::EncoderItem ArgsEncoder::EI_JS("js", encode_js_);
  const ArgsEncoder::EncoderItem ArgsEncoder::EI_BASE64("base64", encode_base64_);
  ...
  void ArgsEncoder::set_callback(ArgsCallback* args_container) noexcept
  {
    args_container_ = args_container;
  }
    ...
    bool value) const /*throw (eh::Exception)*/
  {
    ...
    if (ENCODE_)
    {
      ...
      if (pos != SubString::NPOS)
      {
        ValueEncoder found = encoder_holder.get_value_encoder(key_val.substr(0, pos));
        if (found)
        {
          encoder = found;
    ...
    /*throw (TextTemplException, eh::Exception)*/
  {
    try
    {
      ...
      {
        Stream::Error ostr;
        ostr << FNS << "failed to initialize with file " << fname_ << ": " << ex.what();
        throw TextTemplException(ostr);
      }
    }
    ...
    {
      Stream::Error ostr;
      ostr << FNS << "failed to open file '" << fname_ << "': " << ex.what();
      throw TextTemplException(ostr);
    }
  }
}
