...
namespace String::International
{
  const iconv_t Convertion::INVALID_ = reinterpret_cast<iconv_t>(-1);

  void Convertion::set_encodings(const char* to_code, const char* from_code)
    /*throw (NotSupported)*/
  {
    if (fd_ != INVALID_)
    {
    ...
    {
      Stream::Error ostr;
      ostr << FNS << "Encoding to '" << to_code << "' from '" << from_code << "' not supported";
    ...
    std::string& out) /*throw (eh::Exception, BadSequence)*/
  {
    if (count)
    {
      ...
      out.resize(left_out);
      out_buf = &out[0];
      while (iconv(fd_, &in_buf, &left_in, &out_buf, &left_out) == static_cast<size_t>(-1))
      {
        if (errno == E2BIG)
        {
