...
{
  inline size_t strlcpy(char* dst, const char* src, size_t size) noexcept
  {
  ...
  inline size_t strlcat(char* dst, const char* src, size_t size) noexcept
  {
    const char* const saved_src = src;
  ...
  inline constexpr size_t base64mod_encoded_size(size_t original_size, bool padding) noexcept
  {
  ...
  inline constexpr size_t base64mod_max_decoded_size(size_t original_size) noexcept
  {
    return (original_size + 3) / 4 * 3;
  }

  inline constexpr size_t base64mod_fill_size(size_t original_size) noexcept
  {
    return (8 >> (original_size % 3)) & 6;
  }

  inline size_t append(char* buffer, size_t size, const char* str) noexcept
  {
    size_t length = strlcpy(buffer, str, size);
    return length < size ? length : size;
  }

  inline size_t append(char* buffer, size_t size, char* str) noexcept
  {
    return append(buffer, size, const_cast<const char*>(str));
  }

  inline size_t append(char* buffer, size_t size, const SubString& str) noexcept
  {
    //assert(size);
    if (str.size() >= size)
    {
  ...
  inline size_t append(char* buffer, size_t size, const std::string& str) noexcept
  {
    return append(buffer, size, SubString(str));
  }

  template <typename Integer>
  size_t append(char* buffer, size_t size, Integer integer) noexcept
  {
  ...
  inline void concat(char* buffer, size_t size) noexcept
  {
    (void)size;
    assert(size);
    *buffer = '\0';
...
namespace String
{
  ...
  inline Fmt::Fmt(const char* fmt, ...) noexcept
  {
    int n;
    std::va_list ap;
    va_start(ap, fmt);
  ...
  inline Fmt::operator const char*() const noexcept
  {
    if (text_)
    {
      return text_;
...
namespace String::StringManip
{
  inline void wchar_to_utf8(wchar_t src, std::string& str)
    /*throw (eh::Exception)*/
  {
