...
namespace String::SequenceAnalyzer
{
  ...
  using UIntRanges = Generics::CompressedSet<unsigned>;
  using ShieldMap = std::map<char, std::string>;
...
namespace String::SequenceAnalyzer
{
  ...
  inline CharPair::CharPair() noexcept
    : initialized_(false)
  {
  }

  inline CharPair::CharPair(char single) noexcept
    : initialized_(true), first_(single), second_(single)
  {
  }

  inline CharPair::CharPair(char first, char second) noexcept
    : initialized_(true), first_(first), second_(second)
  {
  }

  inline bool CharPair::initialized() const noexcept
  {
    return initialized_;
  }

  inline char CharPair::first() const noexcept
  {
    return first_;
  }

  inline char CharPair::second() const noexcept
  {
    return second_;
  }
  ...
  inline CharSet::CharSet() noexcept
