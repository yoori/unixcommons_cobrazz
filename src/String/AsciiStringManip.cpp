...
namespace String::AsciiStringManip
{
  ...
  namespace Category
  {
    CharTable::CharTable(const char* str, bool check_zero) noexcept
    {
    ...
    CharTable::CharTable(const CharTable& first, const CharTable& second) noexcept
    {
      for (int i = 0; i < 256; i++)
      {
      ...
      const CharTable& third) noexcept
    {
      for (int i = 0; i < 256; i++)
      {
        table_[i] = first.table_[i] || second.table_[i] || third.table_[i];
      }
    }
  }
}
