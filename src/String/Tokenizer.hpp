...
{
  using TokenizerDefaultSeparators = const AsciiStringManip::Char3Category<' ', '\n', '\t'>;
  ...
  namespace Helper
  {
    ...
    {
      explicit SplitterState(const SubString& str) /*throw (eh::Exception)*/;
    ...
    struct GetToken
    {
      ...
      static bool get_token(SplitterState& state, SubString& token)
        /*throw (eh::Exception)*/;
    };
  ...
  template <typename Category = TokenizerDefaultSeparators, const bool EMPTY = false>
  class Splitter
  {
  public:
    /**
     ...
     */
    explicit Splitter(const SubString& str)
      /*throw (eh::Exception)*/;
  ...
  using CharSplitter = Splitter<const AsciiStringManip::CharCategory&>;

  using SplitColon = Splitter<AsciiStringManip::SepColon>;
  using SplitComma = Splitter<AsciiStringManip::SepComma>;
  using SplitPeriod = Splitter<AsciiStringManip::SepPeriod>;
  using SplitMinus = Splitter<AsciiStringManip::SepMinus>;
  using SplitSemCol = Splitter<AsciiStringManip::SepSemCol>;
  using SplitAmp = Splitter<AsciiStringManip::SepAmp>;
  using SplitSpace = Splitter<AsciiStringManip::SepSpace>;
  using SplitEq = Splitter<AsciiStringManip::SepEq>;
  using SplitSlash = Splitter<AsciiStringManip::SepSlash>;
  using SplitHash = Splitter<AsciiStringManip::SepHash>;
  using SplitBar = Splitter<AsciiStringManip::SepBar>;
  using SplitNL = Splitter<AsciiStringManip::SepNL>;
  using SplitTab = Splitter<AsciiStringManip::SepTab>;
...
namespace String::StringManip
{
  namespace Helper
  {
    ...
    template <typename Category>
    SplitterState<Category>::SplitterState(const SubString& str, Category category)
      /*throw (eh::Exception)*/
      : category(category),
        str(str.begin()), END(str.end()), error(false)
    {
    ...
    template <const bool EMPTY>
    template <typename SplitterState>
    bool GetToken<EMPTY>::get_token(SplitterState& state, SubString& token)
      /*throw (eh::Exception)*/
    {
      ...
      if ((state.separator = state.category.find_owned(state.str, state.END, &octets_length)))
      {
    ...
    template <typename SplitterState>
    bool GetToken<false>::get_token(SplitterState& state, SubString& token)
      /*throw (eh::Exception)*/
    {
      if (const char* begin = state.category.find_nonowned( state.str, state.END))
      {
  ...
  template <typename Category, const bool EMPTY>
  Splitter<Category, EMPTY>::Splitter(const SubString& str, Category category)
    /*throw (eh::Exception)*/
    : state_(str, category)
  {
  }
  ...
  template <typename Category, const bool EMPTY>
  bool Splitter<Category, EMPTY>::get_token(SubString& token)
    /*throw (eh::Exception)*/
  {
  ...
  template <typename Category, const bool EMPTY>
  const char* Splitter<Category, EMPTY>::get_separator() const noexcept
  {
    return state_.separator;
  }

  template <typename Category, const bool EMPTY>
  bool Splitter<Category, EMPTY>::is_error() const noexcept
  {
    return state_.error;
  }
  ...
  inline Tokenizer::Tokenizer(const String::SubString& str, const char* symbols)
    /*throw (eh::Exception)*/
