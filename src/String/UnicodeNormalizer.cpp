...
namespace
{
  ...
  inline
  wchar_t* decompose(const uint64_t* body, wchar_t wch, wchar_t start, wchar_t* output) noexcept
  {
    start = wch - start;
  ...
  template <typename Table>
  wchar_t* decompose(const Table* body, wchar_t wch, wchar_t start, wchar_t* output) noexcept
  {
    wch = body[wch - start];
    if (!wch)
    {
...
{
  wchar_t* hangul_decompose(wchar_t wch, wchar_t* output) noexcept
  {
  ...
  wchar_t* decompose_2008(wchar_t wch, wchar_t* output) noexcept
  {
    if (wch <= 0x33FF)
    {
        ...
        if (wch < 0xAC00)
        {
          ...
          else
          {
            ...
            else
            {
              ...
              {
                // A640-ABFF
                return decompose(IDNA2008Decomposition::MAPPING_A640_ABFF, wch, 0xA640, output);
              }
            }
          }
        }
          ...
          else
          {
            ...
            if (wch < 0x10400)
            {
              ...
              else
              {
                ...
                {
                  // FFEF-103FF
                  return decompose( IDNA2008Decomposition::MAPPING_FFEF_103FF, wch, 0xFFEF, output);
                }
              }
            }
            else
        ...
        if (wch < 0x1D800)
        {
          ...
          if (wch < 0x13000)
          {
            ...
            else
            {
              ...
              {
                // 1239A-12543
                return decompose( IDNA2008Decomposition::MAPPING_1239A_12543, wch, 0x1239A, output);
              }
              else
              {
                // 12544-12FFF
  ...
  wchar_t* decompose_2003(wchar_t wch, wchar_t* output) noexcept
  {
    ...
    const uint32_t* const ELEM = std::lower_bound(MAP, END, wch, std::less<uint32_t>());
  ...
  int get_non_zero_combining_class(uint32_t wch) noexcept
  {
    return (*Combining::COMBINING_CLASS_INDEX[wch >> 8])[wch & 0xFF];
  }

  int get_combining_class(uint32_t wch) noexcept
  {
    return wch > 0x1D244 ? 0 : get_non_zero_combining_class(wch);
  }

  bool is_starter(uint32_t wch) noexcept
  {
    return get_combining_class(wch) == 0;
  }

  bool canonical_order(wchar_t a, wchar_t b) noexcept
  {
  ...
  unsigned hash(wchar_t starter, wchar_t combiner) noexcept
  {
  ...
  wchar_t* compose_string(wchar_t* first, const wchar_t* last) noexcept
  {
   ...
   */
  bool
  pair_compose(wchar_t& maybe_starter, int& last_class, wchar_t*& first, wchar_t*& output) noexcept
  {
   ...
   */
  wchar_t* compose(wchar_t*& first, const wchar_t* last, wchar_t* output) noexcept
  {
    *output = *first++;
    if (first == last)
    {
    ...
    while (first < last && pair_compose(maybe_starter, last_class, first, output))
    {
    }
