...
namespace String::TextTemplate
{
  ...
  public:
     ...
     * @exception eh::Exception std::exception.
     */
    std::string instantiate(const ArgsCallback& args) const
     ...
     * @exception eh::Exception std::exception.
     */
    void keys(const ArgsCallback& args, Keys& keys) const
      /*throw (UnknownName, TextTemplException, eh::Exception)*/;
     ...
     */
    bool empty() const /*throw (eh::Exception)*/;
  ...
  private:
    ...
    public:
       ...
       */
      virtual void append_value(const ArgsCallback& callback, std::string& dst) const
        /*throw (eh::Exception)*/ = 0;
       ...
       */
      virtual std::string key(const ArgsCallback& callback) const
        /*throw (eh::Exception)*/ = 0;
    };
  ...
  private:
    using Items = ReferenceCounting::Deque<Item_var>;
  ...
  {
    static const SubString& real_key(const SubString& key) noexcept;

    template <typename Iterator>
    static std::string value(const Iterator& itor)
      /*throw (eh::Exception)*/;
  };
  ...
  {
    static std::string real_key(const SubString& key) /*throw (eh::Exception)*/;
  };
  ...
  public:
     ...
     */
    explicit ArgsContainer(const Container* cont) noexcept;
  ...
  public:
     ...
     */
    explicit DefaultValue(const ArgsCallback* callback) noexcept;
  ...
  {
  public:
    using ValueEncoder = void (*)(std::string&& value, std::string& encoded);
    ...
    public:
       ...
       */
      explicit EncoderItem(ValueEncoder encode) noexcept;
       ...
       */
      ValueEncoder get_encoder_() const noexcept;
  ...
  public:
    ...
    static const EncoderItem EI_BASE64;
     ...
     */
    void set_callback(ArgsCallback* args_container) noexcept;
  ...
  protected:
    using ValueContainer = Generics::GnuHashTable<
      Generics::StringHashAdapter, std::string>;
  ...
  public:
     ...
     */
    explicit UpdateStrategy(const char* fname) /*throw (eh::Exception)*/;

    /**
     * Destructs UpdateStrategy object
     */
    virtual ~UpdateStrategy() noexcept;
     ...
     */
    void update() /*throw (TextTemplException, eh::Exception)*/;
     ...
     */
    virtual SubString start_lexeme() const /*throw (eh::Exception)*/ = 0;
     ...
     */
    virtual SubString end_lexeme() const /*throw (eh::Exception)*/ = 0;
...
namespace String::TextTemplate
{
  ...
  inline ArgsCallback::~ArgsCallback() noexcept
  {
  }
  ...
  inline bool Basic::empty() const /*throw (eh::Exception)*/
  {
    return items_.empty();
  }
  ...
  inline IStream::IStream() noexcept
  {
  }
  ...
  inline const SubString& ArgsContainerAdapter::real_key(const SubString& key) noexcept
  {
    return key;
  }

  template <typename Iterator>
  std::string ArgsContainerAdapter::value(const Iterator& itor)
    /*throw (eh::Exception)*/
  {
    return itor->second;
  }
  ...
  inline std::string ArgsContainerStringAdapter::real_key(const SubString& key)
    /*throw (eh::Exception)*/
  {
    return key.str();
  }
  ...
  ArgsContainer<Container, Adapter>::ArgsContainer(const Container* cont) noexcept
    : cont_(cont)
  {
  }
    ...
    /*throw (eh::Exception)*/
  {
    ...
    typename Container::const_iterator it = cont_->find(Adapter::real_key(key));
    if (it == cont_->end())
    {
      return false;
    }
  ...
  inline ArgsEncoder::EncoderItem::EncoderItem(ValueEncoder encoder) noexcept
    : encoder_(encoder)
  {
  }

  inline ArgsEncoder::ValueEncoder ArgsEncoder::EncoderItem::get_encoder_() const noexcept
  {
    return encoder_;
  }
  ...
  inline UpdateStrategy::UpdateStrategy(const char* fname)
    /*throw (eh::Exception)*/
    : fname_(fname ? fname : "")
  {
  }

  inline UpdateStrategy::~UpdateStrategy() noexcept
  {
  }

  inline UpdateStrategy::Buffer& UpdateStrategy::get() noexcept
  {
