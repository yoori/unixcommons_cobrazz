...
namespace
{
  void free_nlpir(void* nlpir) noexcept;
...
namespace Language::Segmentor
{
  namespace Chineese
  {
      ...
      /*throw (UniqueException, SegmException)*/
    {
      ...
      try
      {
        std::ifstream in((path ? std::string(path) + "/" : std::string()) + "Data/NLPIR.code");
        if (in)
        {
          file_exists = true;
    ...
    const char* NlpirSegmentor::put_spaces_(const char* phrase, size_t phrase_len)
      /*throw (eh::Exception)*/
    {
      ...
      const char* r = nlpir->ParagraphProcess( std::string(phrase, phrase_len).c_str(), 0);
      if (!r)
      {
        Stream::Error ostr;
