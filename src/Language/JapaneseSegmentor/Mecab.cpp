...
namespace
{
  ...
  const char JAPANESE_RANGES[] = "\xe2\xba\x80-\xe2\xbb\xb3"
  ...
  using NotJapanese = const String::StringManip::InverseCategory<String::Utf8Category>;
...
namespace Language::Segmentor
{
  namespace Japanese
  {
      ...
      : tagger_(0)
    {
      ...
      {
        Stream::Error error;
        error << FNS << "Can't init mecab tagger: " << MeCab::getTaggerError();
        throw SegmException(error);
      }
    }

    inline MecabSegmentor::MecabTagger_::~MecabTagger_() noexcept
    {
    }

    inline bool MecabSegmentor::MecabTagger_::empty() const noexcept
    {
    ...
    inline
    void
    MecabSegmentor::MecabTagger_::parse_to(Target& target, const char* phrase, size_t phrase_len)
      /*throw (eh::Exception)*/
    {
      ...
      {
        Stream::Error error;
        error << FNS <<  "Can't parse phrase \"" << String::SubString(phrase, phrase_len)
              << "\". Description: " << tagger_->what();
        throw SegmException(error);
      }
      while (node)
      {
    ...
    inline MecabSegmentor::MecabTagger_var_ MecabSegmentor::get_tagger_() const
      /*throw (SegmException)*/
    {
    ...
    inline void MecabSegmentor::put_tagger_(MecabTagger_var_& tagger) const
      /*throw (SegmException)*/
    {
      try
      {
        ...
        waiting_sem_(0), waiting_num_(0)
    {
      try
      {
        ...
        buf << "-r " << (config_file ? config_file : MECAB_DEFAULT_CONFIG) <<
    ...
    MecabSegmentor::MecabTagger_var_ MecabSegmentor::init_new_tagger_() const
      /*throw (SegmException)*/
    {
      try
      {
      ...
      size_t phrase_len) const /*throw (SegmException)*/
    {
      try
      {
        ...
        String::StringManip::Splitter<const NotJapanese&> tokenizer( input, NOT_JAPANESE);
    ...
    void
    MecabSegmentor::put_spaces(std::string& result, const char* phrase, size_t phrase_len) const
      /*throw (SegmException)*/
    {
      try
      {
    ...
    bool MecabSegmentor::is_valid_utf8_(const char* str, size_t str_len) const noexcept
    {
    ...
    void MecabSegmentor::segmentation(WordsList&, const char*, size_t) const
      /*throw (SegmException)*/
    {
    }

    void MecabSegmentor::put_spaces(std::string&, const char*, size_t) const
      /*throw (SegmException)*/
    {
    }
#endif
