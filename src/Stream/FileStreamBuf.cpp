...
namespace Stream::File
{
  ...
  InStreamBuf::InStreamBuf(IO* io, size_t buffer_size, size_t put_back_size)
  ...
  InStreamBuf::int_type InStreamBuf::underflow() /*throw (Underflow, eh::Exception)*/
  {
    assert (gptr() == egptr());

    size_t number_to_put_back = std::min(static_cast<size_t>(gptr() - eback()), PUT_BACK_SIZE_);
    ...
    size_t bytes_read = IO_->read(in_buffer_.get() + PUT_BACK_SIZE_, BUFFER_SIZE_- PUT_BACK_SIZE_);
    if (bytes_read == 0)
    {
  ...
  OutStreamBuf::int_type OutStreamBuf::overflow(int_type c) /*throw (Overflow, eh::Exception)*/
  {
    assert (pptr() != pbase());
