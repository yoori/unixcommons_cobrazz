...
namespace Stream::MemoryStream
{
  ...
  public:
    ...
    using Position = typename Traits::pos_type;
    using Offset = typename Traits::off_type;
  ...
  protected:
    ...
    virtual Position seekpos(Position pos, std::ios_base::openmode which)
      /*throw (eh::Exception)*/;

    virtual Int underflow() noexcept;
  };
  ...
  public:
     ...
     */
    virtual ~OutputMemoryBuffer() noexcept;
  ...
  protected:
    ...
    virtual Int overflow(Int c = Traits::eof()) /*throw (eh::Exception)*/;
  ...
  private:
     ...
     */
    bool extend() /*throw (eh::Exception)*/;
  ...
  {
  public:
    using Elem = typename Buffer::char_type;

    using SubString = typename String::BasicSubString<const Elem,
      String::CharTraits<typename std::remove_const<Elem>::type>,
        String::CheckerNone<Elem> >;
     ...
     */
    SubString str() const /*throw (eh::Exception)*/;
  ...
  {
  private:
    using Holder = MemoryBufferHolder<InputMemoryBuffer<Elem, Traits> >;
    using Stream = std::basic_istream<Elem, Traits>;
  ...
  public:
    ...
    InputMemoryStream( const String::BasicSubString<Char, STraits, Checker>& str)
      /*throw (eh::Exception)*/;
  ...
  {
  private:
    using Holder = MemoryBufferHolder<
      OutputMemoryBuffer<Elem, Traits, Allocator, AllocatorInitializer> >;
    using Stream = std::basic_ostream<Elem, Traits>;
  ...
  namespace Allocator
  {
    ...
    {
    public:
      using Allocator = std::allocator<Elem>;
      using AllocatorTraits = std::allocator_traits<Allocator>;
      using Pointer = typename AllocatorTraits::pointer;
      using Size = typename AllocatorTraits::size_type;
       ...
       */
      Pointer allocate(Size size, const void* = 0) noexcept;
       ...
       * @param size should be equal to SIZE
       */
      void deallocate(Pointer ptr, Size size) noexcept;
    ...
    public:
       ...
       */
      explicit SimpleBuffer(Elem* buffer) noexcept;
    };
    ...
    {
    public:
      explicit ArrayBuffer(Initializer initializer = Initializer()) noexcept;
    ...
    public:
       ...
       * Constructor
       */
      explicit SimpleStack(size_t allocator_initializer) noexcept;
    };
  }
}
...
namespace Stream
{
  ...
  using Parser = MemoryStream::InputMemoryStream<char>;
  using WParser = MemoryStream::InputMemoryStream<wchar_t>;

  // Dynamic memory output stream with preallocation
  using Dynamic = MemoryStream::OutputMemoryStream<char>;
  ...
  {
  private:
    using Allocator = MemoryStream::Allocator::SimpleBuffer<char, SIZE>;
  ...
  public:
     ...
     */
    explicit Buffer(char* buffer) noexcept;
   ...
   * enough size
   */
  using FileName = Buffer<MAXPATHLEN>;
}
