...
namespace Generics::Allocator
{
  ...
  Base* Base::get_default_allocator() /*throw (eh::Exception)*/
  {
  ...
  inline void Base::align_(size_t& number, size_t mask) noexcept
  {
    number += (-number) & mask;
  }

  size_t Base::cached() const /*throw (eh::Exception)*/
  {
    return 0;
  }

  void Base::print_cached(std::ostream& ostr) const /*throw (eh::Exception)*/
  {
    ostr << '0';
  }
  ...
  Base::Pointer Default::allocate(size_t& size)
  ...
  void Default::deallocate(Pointer ptr, size_t size) noexcept
  {
    (void)size;
    assert(!(size & MASK_));
  ...
  inline void VarSizeList::memory_block_delete_(MemoryBlock& mb) noexcept
  {
    delete [] (static_cast<unsigned char*>(mb.second));
  }

  Base::Pointer VarSizeList::allocate(size_t& size)
    /*throw (eh::Exception, OutOfMemory)*/
  {
    align_(size, MASK_);
    {
  ...
  void VarSizeList::deallocate(Pointer ptr, size_t size) noexcept
  {
    try
    {
        ...
        else
        {
          ...
          if (it != pool_memory_blocks_.begin())
          {
            ...
            *it = save;
            ...
            if (it != pool_memory_blocks_.begin())
            {
              pool_memory_blocks_.splice( pool_memory_blocks_.begin(), pool_memory_blocks_, it);
            }
          }
        }
      }
  ...
  size_t VarSizeList::cached() const /*throw (eh::Exception)*/
  {
    Sync::PosixGuard lock(mutex_);
    return cached_;
  }

  void VarSizeList::print_cached(std::ostream& ostr) const
    /*throw (eh::Exception)*/
  {
    ostr << cached();
  }
  ...
  Base::Pointer ConstSizeArray::allocate(size_t& size)
    /*throw (eh::Exception, OutOfMemory)*/
  {
    if (size > BLOCK_SIZE_)
    {
  ...
  void ConstSizeArray::deallocate(Pointer ptr, size_t size) noexcept
  {
    (void)size;
...

    {
      ...
      {
        pool_memory_blocks_[blocks_in_pool_++].unsafe_reset( static_cast<unsigned char*>(ptr));
        return;
      }
    }
    delete [] static_cast<unsigned char*>(ptr);
  ...
  size_t ConstSizeArray::cached() const /*throw (eh::Exception)*/
  {
  ...
  void ConstSizeArray::print_cached(std::ostream& ostr) const
    /*throw (eh::Exception)*/
  {
    ...
    ostr << BLOCK_SIZE_ << ':' << blocks_in_pool << '(' << hits << '+' << misses << ')';
  }
  ...
  Universal::Universal(
    ...
    : DEFAULT_THRESHOLD_LOW_(default_threshold_low),
      ...
      default_allocator_( ReferenceCounting::add_ref(get_default_allocator())),
      ...
      statistics_(statistics_limit + 1)
  {
    ...
    std::fill(statistics_.get(), statistics_.get() + statistics_limit + 1, 0);
  }
  ...
  inline Base_var Universal::get_allocator_(size_t size) noexcept
  {
  ...
  Base::Pointer Universal::allocate(size_t& size) /*throw (eh::Exception, OutOfMemory)*/
  {
  ...
  void Universal::deallocate(Pointer ptr, size_t size) noexcept
  {
    get_allocator_(size)->deallocate(ptr, size);
  }

  size_t Universal::cached() const /*throw (eh::Exception)*/
  {
  ...
  void Universal::print_cached(std::ostream& ostr) const /*throw (eh::Exception)*/
  {
    ostr << cached() << " D:";
    ...
    {
      static const char SCALE[] = "123456789"
  ...
  Base::Pointer Align::allocate(size_t& size)
    /*throw (eh::Exception, OutOfMemory)*/
  {
    align_(size, MASK_);
    void* ptr;
  ...
  void Align::deallocate(Pointer ptr, size_t size) noexcept
  {
    (void)size;
    assert(!(size & MASK_));
    free(ptr);
