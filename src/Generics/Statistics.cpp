...
namespace Generics::Statistics
{
  ...
  void Collection::activate_object()
  ...
  void Collection::deactivate_object()
  ...
  void Collection::add(const char* id, StatSink* stat, DumpPolicy* policy)
    /*throw (InvalidArgument, Exception, eh::Exception)*/
  {
    if (id == 0)
    {
    ...
    {
      Stream::Error ostr;
      ostr << FNS << "Generics::Statistics::Collection::Item::Exception: " << e.what();
      throw InvalidArgument(ostr);
    }
  }

  void Collection::dump(std::ostream& ostr)
    /*throw (Exception, eh::Exception)*/
  {
    ReadGuard_ guard(lock_);

    for (ItemMap::iterator item = items_.begin(); item != items_.end(); ++item)
    {
  ...
  void Collection::Item::dump(std::ostream& ostr)
    /*throw (eh::Exception)*/
  {
  ...
  std::string Collection::Item::current_time()
    /*throw (eh::Exception)*/
  {
    return Time::get_time_of_day().get_local_time().format( "%a %d %b %Y %H:%M:%S");
  }
}
