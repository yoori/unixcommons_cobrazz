...
namespace Generics::DirSelect
{
   ...
   */
  using ErrorHandler = void (*)(const char* full_path);
...
#undef DEFINE_MODE_TEST
  ...
  public:
     ...
     */
    bool operator ()(const char* full_path, const struct stat& st) const
      /*throw (eh::Exception)*/;
  ...
  public:
     ...
     * to the functor
     */
    FunctorWrapper(Functor& predicate, bool file_name_only, bool result) noexcept;
  ...
  wrap_functor(Functor& functor, bool file_name_only = false, bool result = true) noexcept;
   ...
   */
  const char* file_name(const char* full_path) noexcept;

  /**
   * Default handler for opendir(2) fail
   */
  void default_failed_to_open_directory(const char* full_path)
    /*throw (Exception)*/;
   ...
   */
  void default_failed_to_stat_file(const char* full_path)
    /*throw (Exception)*/;
  ...
  directory_selector(const char* path, const Predicate& predicate,
    ...
    ErrorHandler failed_to_open_directory = default_failed_to_open_directory,
  ...
  public:
     ...
     */
    explicit ListCreator(Iterator iterator) noexcept;
     ...
     * @param st file information
     */
    bool operator ()(const char* full_path, const struct stat& st)
      /*throw (eh::Exception)*/;
  ...
  template <typename Iterator>
  ListCreator<Iterator> list_creator(Iterator iterator) /*throw (eh::Exception)*/;
}
...
namespace Generics::DirSelect
{
   ...
   * NamePattern class
   */
  inline NamePattern::NamePattern(const char* pattern) /*throw (eh::Exception)*/
    : pattern_(pattern)
  {
  }

  inline bool NamePattern::operator ()(const char* full_path, const struct stat&) const
    /*throw (eh::Exception)*/
  {
    return !fnmatch(pattern_.c_str(), file_name(full_path), FNM_PATHNAME);
  }

  inline const char* file_name(const char* full_path) noexcept
  {
  ...
  template <typename Functor>
  inline
  FunctorWrapper<Functor>::FunctorWrapper( Functor& functor, bool file_name_only, bool result)
    noexcept
  ...
  template <typename Functor>
  bool FunctorWrapper<Functor>::operator ()( const char* full_path, const struct stat& st) const
    /*throw (eh::Exception)*/
  {
    return functor_(file_name_only_ ? file_name(full_path) : full_path, st), result_;
  }

  template <typename Functor>
  FunctorWrapper<Functor> wrap_functor(Functor& functor, bool file_name_only, bool result) noexcept
  {
    return FunctorWrapper<Functor>(functor, file_name_only, result);
  }

  inline void default_failed_to_open_directory(const char* full_path)
    /*throw (Exception)*/
  {
    eh::throw_errno_exception<FailedToOpenDirectory>(FNE, "'", full_path, "'");
  }

  inline void default_failed_to_stat_file(const char* full_path)
    /*throw (Exception)*/
  {
    eh::throw_errno_exception<FailedToStatFile>(FNE, "'", full_path, "'");
  }

  namespace DirectoryFilterHelper
  {
    using StatFunc = int (*)(const char*, struct stat*);
    ...
    inline DirPtr::DirPtr(const char* path) noexcept
    {
      dir = opendir(path);
    }

    inline DirPtr::~DirPtr() noexcept
    {
      if (dir)
      {
        closedir(dir);
    ...
    inline DIR* DirPtr::get() const noexcept
    {
      return dir;
    }
      ...
      ErrorHandler failed_to_stat_file) /*throw (eh::Exception)*/
    {
      ...
      {
        if (String::StringManip::strlcpy(name, entry->d_name, NAME_MAX) >= NAME_MAX)
        {
          //TODO
        }
  ...
  directory_selector(const char* path, const Predicate& predicate,
    ...
    ErrorHandler failed_to_stat_file) /*throw (eh::Exception)*/
  {
    DirectoryFilterHelper::StatFunc stat_func = resolve_links ? stat : lstat;
  ...
  {
    ErrorHandler failed_to_open_directory = flags & DSF_EXCEPTION_ON_OPEN ?
      default_failed_to_open_directory : 0;
    ErrorHandler failed_to_stat_file = flags & DSF_EXCEPTION_ON_STAT ?
    ...
    if (recursive)
    {
      if (only_regular)
      {
        directory_selector(path,
          true2(and2(and2(IsRegular(), NamePattern(mask)), wrap_functor(functor, file_name_only))),
          resolve_links, failed_to_open_directory, failed_to_stat_file);
      }
      else
      {
        directory_selector(path,
          true2(and2(NamePattern(mask), wrap_functor(functor, file_name_only))),
          resolve_links, failed_to_open_directory, failed_to_stat_file);
      }
    }
    else
    {
      ...
      {
        directory_selector(path,
          and2(and2(IsRegular(), NamePattern(mask)), wrap_functor(functor, file_name_only)),
          resolve_links, failed_to_open_directory, failed_to_stat_file);
      }
      else
      {
        directory_selector(path,
          and2(NamePattern(mask), wrap_functor(functor, file_name_only, false)),
          resolve_links, failed_to_open_directory, failed_to_stat_file);
      }
    }
  }
  ...
  template <typename Iterator>
  bool ListCreator<Iterator>::operator ()( const char* full_path, const struct stat&)
    /*throw (eh::Exception)*/
  {
    *iterator_++ = full_path;
    return true;
