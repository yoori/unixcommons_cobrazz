...
namespace Generics::DecimalHelper
{
  ...
  struct Splitter
  {
    static void split(Integer& integer, bool& negative) noexcept;
  };
  ...
  template <typename Integer, const bool UNSIGNED>
  void Splitter<Integer, UNSIGNED>::split(Integer& integer, bool& negative) noexcept
  {
    if (integer < 0)
    {
      negative = true;
  ...
  template <typename Integer>
  void Splitter<Integer, true>::split(Integer& /*integer*/, bool& negative) noexcept
  {
    negative = false;
  }

  template <typename Integer>
  void split(Integer& integer, bool& negative) noexcept
  {
    Splitter<Integer, !std::numeric_limits<Integer>::is_signed>::split( integer, negative);
  }
  ...
  Base assemble_decimal(unsigned digits, const unsigned char* num) noexcept
  {
    Base ret = 0;
   ...
   */
  template <typename Base>
  void disassemble_decimal(unsigned digits, Base elem, unsigned char* num) noexcept
  {
    if (digits)
    {
      unsigned i = digits - 1;
   ...
   */
  template <typename Base>
  Base pow10(unsigned pow) noexcept
  {
    if (pow < 20)
    {
  ...
  inline bool exceeds(unsigned long long a, unsigned long long b) noexcept
  {
    return a >= b;
  }

  template <typename Iterator>
  void skip(Iterator& cur, Iterator& end, std::ios_base::iostate& iostate) noexcept
  {
    bool eof;
    while (!(eof = ++cur == end) && static_cast<unsigned>(*cur - '0') < 10)
    {
    }
   ...
   */
  template <const unsigned TOTAL, const unsigned FRACTION, typename Base, typename Iterator>
  const char*
  ...
  template <const bool>
  void
  mul(uint64_t factor1, uint64_t factor2, uint64_t base, uint64_t& major, uint64_t& minor) noexcept
  {
...
namespace Generics
{
  ...
  {
  private:
    static_assert(std::numeric_limits<BaseType>::is_integer, "base type must be integer");
    static_assert(std::numeric_limits<BaseType>::is_bounded, "base type must be bounded");
  };

  /**
   */
  template <typename BaseType, const unsigned TOTAL, const unsigned FRACTION>
  ...
  private:
    ...
    static_assert(FRACTION_RANK <= TOTAL_RANK, "FRACTION must be less than or equal to TOTAL");
  ...
  {
  private:
    static_assert(!std::numeric_limits<BaseType>::is_signed, "BaseType must be unsigned");
  ...
  void narrow_decimal(SimpleDecimal& s, const Decimal& d) /*throw (eh::Exception)*/
  {
    static_assert(Decimal::FRACTION_RANK >= SimpleDecimal::FRACTION_RANK,
      "Decimal FRACTION must not be less than SimpleDecimal one");
    static_assert(
      static_cast<unsigned>(std::numeric_limits<int64_t>::digits10) >= SimpleDecimal::TOTAL_RANK,
      "SimpleDecimal TOTAL must not exceed int64_t");
    static_assert(
      static_cast<unsigned>(std::numeric_limits<int64_t>::digits10) >= SimpleDecimal::FRACTION_RANK,
  ...
  SimpleDecimal convert_float(Float value) /*throw (eh::Exception)*/
  {
    ...
    const int64_t POW = DecimalHelper::Pow10<int64_t, SimpleDecimal::FRACTION_RANK>::Value;
    if ((value >= 0 ? value : -value) >= std::numeric_limits<int64_t>::max() / POW)
    {
      Stream::Error ostr;
      ostr << FNS << "provided value is too big";
      throw typename SimpleDecimal::Overflow(ostr);
    }
    return SimpleDecimal(static_cast<int64_t>(value * POW), SimpleDecimal::FRACTION_RANK);
  }
}
