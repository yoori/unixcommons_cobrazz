...
namespace Generics::AppUtils
{
  OptionCont OptionCont::operator ||(const OptionCont& right) noexcept
  {
    ...
    ret.eq_options.insert(ret.eq_options.end(), right.eq_options.begin(), right.eq_options.end());
  ...
  bool Args::ParseState::next_word() noexcept
  {
    ++argv_;
    --argc_;
  ...
  const char* Args::ParseState::current_pos() noexcept
  {
    return arg_pos_;
  }

  void Args::ParseState::current_pos(const char* pos) noexcept
  {
    arg_pos_ = pos;
  }

  /**X Args */
  bool Args::parse_eq_op_(ParseState& parse_state)
  ...
  bool Args::parse_short_opt_seq_(ParseState& parse_state)
  ...
  bool Args::parse_short_opt_(ParseState& parse_state)
    /*throw (Exception, InvalidParam)*/
  {
    const char* cur_opt = parse_state.current_pos();
    ...
    /*throw (Exception, InvalidParam)*/
  {
    if (it->second->require_value())
    {
      ...
      {
        Stream::Error ostr;
        ostr << FNS << "Undefined value after option '" << opt_name << "'";
        throw Exception(ostr);
      }
    ...
    /*throw (eh::Exception, Exception, InvalidParam)*/
  {
    ...
    {
      if (!parse_eq_op_(parse_state) && !parse_short_opt_seq_(parse_state))
      {
        ...
        {
          Stream::Error ostr;
          ostr << FNS << "Unknown option: '" << parse_state.current_pos() << "'";
          throw Exception(ostr);
        }
      }
    }
  ...
  void Args::usage(std::ostream& ostr) const /*throw (eh::Exception)*/
  {
    for (Usage::const_iterator itor(usage_.begin()); itor != usage_.end(); ++itor)
    {
      ostr << itor->second << "\n";
    }
  }
