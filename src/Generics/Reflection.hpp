...
namespace Generics::Reflection
{
  ...
  private:
     ...
     */
    static bool add_info_() /*throw (eh::Exception)*/;
  ...
  public:
    ...
    template <typename T>
    explicit Child(const T& data) /*throw (eh::Exception)*/;
    ...
    Child(const T1& data1, const T2& data2, const T3& data3, const T4& data4, const T5& data5)
      /*throw (eh::Exception)*/;
    ...
    template <typename T>
    Child& operator =(const T& data) /*throw (eh::Exception)*/;
  };
...
namespace Generics::Reflection
{
  ...
  const bool MemberRegistrator<MembersBase, Aggregator,
    NAME, MEMBER, CONST_MEMBER>::INITIALIZER_ = MemberRegistrator<MembersBase, Aggregator,
  ...
  template <typename Inheritance, typename Aggregator>
  bool Base<Inheritance, Aggregator>::add_info_() /*throw (eh::Exception)*/
  {
...
#define REFLECTION_INIT(Aggregator, MembersBase) \
  using ReflectionAggregator = Aggregator; \
  using ReflectionMembersBase = MembersBase; \
  \
  ...
  \
  using ReflectionMembersInfo = std::list<ReflectionMemberInfo>; \
  \
  static \
