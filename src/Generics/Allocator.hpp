...
{
  class Base;
  using Base_var = ReferenceCounting::QualPtr<Base>;
  using FixedBase_var = ReferenceCounting::FixedPtr<Base>;
  using SmartBase_var = ReferenceCounting::SmartPtr<Base>;
  ...
  struct Base : public virtual ReferenceCounting::Interface
  {
     ...
     */
    virtual Pointer allocate(size_t& size) /*throw (eh::Exception, OutOfMemory)*/ = 0;
     ...
     */
    virtual void deallocate(Pointer ptr, size_t size) noexcept = 0;

    /**
     * Approximated cached memory size.
     * @return cached memory size.
     */
    virtual size_t cached() const /*throw (eh::Exception)*/;

    /**
     * Print detailed approximate cached memory information.
     */
    virtual void print_cached(std::ostream& ostr) const /*throw (eh::Exception)*/;

    /**
     * @return application level default allocator. Usually
     * simple new/delete behavior.
     */
    static Base* get_default_allocator() /*throw (eh::Exception)*/;
  ...
  protected:
     ...
     */
    static void align_(size_t& number, size_t mask) noexcept;
  ...
  public:
    ...
    explicit Default(size_t align_code = DEF_ALIGN) noexcept;
     ...
     */
    virtual Pointer allocate(size_t& size) /*throw (eh::Exception, OutOfMemory)*/;
     ...
     */
    virtual void deallocate(Pointer ptr, size_t size) noexcept;
  ...
  public:
     ...
     */
    explicit VarSizeList(size_t align_code, size_t blocks_count)
      /*throw (eh::Exception)*/;
  ...
  protected:
    virtual ~VarSizeList() noexcept;

  private:
    using MemoryBlock = std::pair<size_t, Pointer>;
    using MemBlocks = std::list<MemoryBlock,
      Generics::TAlloc::Aggregated<MemoryBlock, 256>>;

    /**
     * @param mb this memory block will be return to
     * operation system
     */
    static void memory_block_delete_(MemoryBlock& mb) noexcept;
  ...
  public:
     ...
     */
    ConstSizeArray(size_t max_blocks_count, size_t block_size) /*throw (eh::Exception)*/;
  ...
  protected:
    virtual ~ConstSizeArray() noexcept;
  ...
  private:
     ...
     */
    Base_var get_allocator_(size_t size) noexcept;
  };
  ...
  public:
    ...
    explicit Align(size_t ptr_align_code = DEF_PTR_ALIGN, size_t align_code = DEF_ALIGN) noexcept;
  ...
  public:
    ...
    static Base* allocator(const Allocator& allocator = Allocator())
      /*throw (eh::Exception)*/;
  ...
  private:
    explicit Template(Allocator& allocator) /*throw (eh::Exception)*/;
...
namespace Generics::Allocator
{
  ...
  template <typename Allocator>
  typename Template<Allocator>::Pointer Template<Allocator>::allocate(size_t& size)
    /*throw (eh::Exception, OutOfMemory)*/
  {
    ...
    {
      Stream::Error ostr;
      ostr << FNS << "Failed to allocate " << size << " bytes: " << ex.what();
  ...
  template <typename Allocator>
  void Template<Allocator>::deallocate( Pointer ptr, size_t size) noexcept
  {
  ...
  template <typename Allocator>
  void Template<Allocator>::delete_this_() const noexcept
  {
  ...
  template <typename Allocator>
  Base* Template<Allocator>::allocator(const Allocator& allocator)
    /*throw (eh::Exception)*/
  {
