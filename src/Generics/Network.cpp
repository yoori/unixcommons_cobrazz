...
namespace Generics::Network
{
  ...
  void
  Resolver::get_host_by_name(const char* host_name, hostent& addresses, char* buf, size_t buf_size)
    /*throw (Exception)*/
  {
    if (!host_name)
    {
    ...
    hostent* result;
    int error;
    if (gethostbyname_r(host_name, &addresses, buf, buf_size, &result, &error))
    {
    ...
    : addresses_(0)
  {
    ...
    {
      eh::throw_errno_exception<Exception>(FNE, "failed to enum interfaces");
    }
  }
  ...
  bool IsLocalInterface::check_host_name(const char* host_name) const
      ...
      UnresolvableAddress)*/
  {
    ...
    for (int i = 0; (address = reinterpret_cast<const in_addr*>(addresses.h_addr_list[i])); i++)
    {
  ...
  uint32_t IsLocalInterface::ip_address(const sockaddr_in* address) noexcept
  {
