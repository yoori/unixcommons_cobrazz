...
namespace Generics::CRC
{
   ...
   * @param size its size
   */
  uint32_t quick(uint32_t crc, const void* data, size_t size) noexcept;
   ...
   * @param size its size
   */
  uint32_t reversed(uint32_t crc, const void* data, size_t size) noexcept;
}
...
namespace Generics::CRC
{
  ...
  inline uint32_t quick(uint32_t crc, const void* data, size_t size) noexcept
  {
  ...
  inline uint32_t reversed(uint32_t crc, const void* data, size_t size) noexcept
  {
    const uint8_t* udata = static_cast<const uint8_t*>(data);
    crc = ~crc;
    while (size-- > 0)
